...
public class Log {
  ...
  public static void initializeState(boolean on, boolean useConsole, boolean alsoLogInfo) {
    ...
    if (b != Log.on) {
      Log.log("Logging is now " + (Log.on ? "on" : "off"));
    }
  ...
  //@ requires msg != null;  
  //@ modifies log.content;
  public static void errorlog(final String msg, final /*@ non_null @*/ Throwable e) {
    log.ierrorlog(msg, e);
  }
  ...
  //@ modifies \nothing;
  //@ ensures \result.consoleName == name;
  private Log(final String name, final Plugin plugin) {
  ...
  //@ requires msg != null;
  //@ modifies content;
  public void ilog(final String msg) {
    if (useConsole) {
  ...
  //@ requires msg != null;
  //@ modifies content;
  public void ierrorlog(final String msg, final Throwable e) {
    ...
    pluginLog.log(
        new Status(Status.ERROR, 
            ...
            Status.OK, msg, e));
    if (useConsole) {
      final MessageConsoleStream cs = getConsoleStream();
      final Color c = cs.getColor();
  ...
  public static class StreamToConsole extends OutputStream {
     ...
     * given console.
     * @param cons The console to write to via the OutputStream
     */
    public StreamToConsole(final MessageConsoleStream cons) {
     ...
     */
    private void dump(final boolean end) {
      if (len == 0) {
        return;
      }
