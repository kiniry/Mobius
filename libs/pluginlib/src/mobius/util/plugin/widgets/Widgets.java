...
public class Widgets {
  ...
  public static class HComposite extends Composite {
     ...
     */
    //@ requires cols > 0;
    public HComposite(final /*@ non_null */ Composite parent, final int cols) {
  ...
  public static class VComposite extends Composite {
     ...
     * @param parent  The container into which to place this new widget
     */
    public VComposite(final /*@ non_null */ Composite parent) {
      super(parent, SWT.NONE);
      final GridLayout layout = new GridLayout();
      layout.numColumns = 1;
      setLayout(layout);
  ...
  public static class LabeledSeparator extends Composite {
     ...
     */
    public LabeledSeparator(final /*@ non_null */ Composite parent, 
  ...
  public static class FileTextField extends VComposite {
    ...
    private final String spaces = tenspaces + tenspaces + tenspaces + 
          tenspaces + tenspaces + tenspaces + tenspaces + 
          tenspaces + tenspaces;
    ...
    private void initComponents(final Composite parent, final String label,
      ...
      if (s.length() < n) {
        s += spaces.substring(0, n - s.length());
      }
      text.setText(s);
      text.setToolTipText(toolTipText);
      // Make the text fixed width and bold
      final Font f = JFaceResources.getFontRegistry().getBold(JFaceResources.TEXT_FONT);
      text.setFont(f);
      final Text t = text;
      
      final Button browse = new Button(hc, SWT.PUSH);
      ...
      browse.addMouseListener(new MouseAdapter() {
        public void mouseUp(final MouseEvent e) {
          ...
          final FileDialog dd = new FileDialog(shell, SWT.OPEN);
          final String result = dd.open();
          if (result != null) {
            t.setText(result);
          }
        }
