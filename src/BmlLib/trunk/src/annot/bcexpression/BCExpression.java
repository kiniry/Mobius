...
/**
 * This class represents an BML expression. It's a superclass for all types of
 * expression. To add a new subexpression create a subclass of this class or one
 * of it's subclasses (eg. AbstractIntExpression for expression that returns
...
public abstract class BCExpression {
	...
	/**
	 * Type of expression, from annot.io.Code interface, used also for .class
	 * file i/o operations.
	 */
	...
	/**
	 * A constructor from AttributeReader. After reading type of expression
	 * (connector) Attribuet reader creates a proper subclass of BCExpression
	 * using this constructor, passing to it connector and AttributeReader
	 * itself. This constructor calls init() method for private fields
	 * initializetion and than read(ar, root) method that reads expression data
	 * ()including it's subexpression) from given AttributeReader.
	 * 
	 * @param ar -
	 *            stream to load from,
	 * @param root -
	...
	/**
	 * This method should be implemented in subclasses to return expression's
	 * String representation. Subclasses should call printCode(conf) method
	 * instead of this method for recursive displaying subecpressions, unless
	 * they want them to be displayed in the same line and without parenthness.
	 * 
	 * @param conf -
	 *            see {@link BMLConfig}.
	...
	/**
	 * Reads the exression from an AttributeReader (except connector, that has
	 ...
	 * @throws ReadAttributeException -
	 *             if connector + stream in <code>ar</code> doesn't represent
	 *             any expression from calling subclass.
	 */
	protected abstract void read(AttributeReader ar, int root)
			throws ReadAttributeException;

	/**
	 * Writes this expression to AttributeWirter. While overriding, don't forget
	 * to write connector first, then other data and finally call
	 * writeSubExpressions(aw) to write all subexpressions recursivly while
	...
	/**
	 * Initialize private data of subclass. While overriding, use this method
	 * instead of initialize private fields in constructor, becouse read()
	 * method is called in spuerclass constructor (from AttributeReader, that
	 * is, before calling subclass constructor).
	 */
	...
	/**
	 * This method should check if all subexpression have correct types and
	 * return type of this expression. You can assume that all subexpressions
	 * are not-null.
	 * 
	...
	/**
	 * Checks if all subexpressions have correct types (recursivly) and return
	 * type of this expression.
	 * 
	 * @return (bmllib's) JavaType of result of this exrpession, or null if it's
	 *         invalid (if one or more of it's subexpression have wrong type or
	 *         are invalid).
	 */
	...
	/**
	 * @return (bmllib's) JavaType of result of this expression (without
	 *         checkign subexpressions).
	 */
	public abstract JavaType1 getType();

	/**
	 * Prints expression as a whole attribute. This method should not be called
	 * by subclasses.
	 * 
	 * @param conf -
	 *            see {@link BMLConfig},
	 * @param usedc - #
	 *            of characters displayed in current line before this expression
	 *            (excluding comment mark -
	 *            annot.textio.IdisplayStyle.comment_*).
	 * @return pretty printed String representation of this expression.
	 */
	...
	/**
	 * Adds parenthness and block marks to the root of this expression.
	 * 
	 * @param conf -
	 *            see {@link BMLConfig}.
	 * @return String representation of expression, without line-breaking.
	 */
	 ...
	 * @param conf -
	 *            see {@link BMLConfig}.
	 * @return debug string representation of expression, with simple
	 *         line-breaking and indention.
	 */
	...
	/**
	 * Prints expression as a string. This method should not be called in
	 * attributes nor subclasses to get full string representation. Use
	 * printLine(conf) in attributes and printCode1(conf) in subclasses.
	 * 
	 * @param conf -
	 *            see {@link BMLConfig}.
	 * @return string representation of expression, without line-breaking.
	...
	/**
	 * Replaces this expression with given one (updates it's parent
	 * subexpression table).<br>
	 * Be sure to call {@link #computeSize(BCExpression, int)} for this
	 * expression (eg. by calling {@link #computeSize()} at the root
	 * expression).
	 * 
	 * @param expr -
	 *            expression to replace this expression.
	 */
	...
	/**
	 * Computes parent node and subtree size for whole subtree. This should be
	 * called ONLY at the root of the expression.
	 * 
	 ...
	 * 
	 * @param parent -
	 *            this expression's parent,
	 * @param chn -
	 *            position (subexpression numer) of this expression in
	 *            <code>parent</code>.
	 * @return size of expression tree.
	 */
	 ...
	 * 
	 * @param arr -
	 *            array to write subtree nodes to. Should be initialized and
	 *            long enought to fit all subtree nodes after <code>pos</code>
	 *            position,
	 * @param pos -
	 *            initial position (index) in <code>arr</code> from with it
	 *            sould be filled in,
	 * @param suffix -
	 *            tree walk order (prefix order for false and suffix order for
	 *            true).
	 * @return next free position in <code>arr</code> (pos + size of subtree).
	 */
	 ...
	 *            tree walk order (prefix order for false and suffix order for
	 *            true).
	 * @return All subtree nodes (recursively) of this expression tree.
	 */
	 ...
	 * 
	 * @param suffix -
	 *            processing order (true ==> suffix order, false ==> prefix
	 *            order),
	 * @param ew -
	 *            visitor (function that will be applied to each node of the
	 *            expression's tree).
	 */
	 ...
	 * @param chn -
	 *            position of parent's child equal that is equal (==) to this
	 *            object,
	 * @param suffix -
	 *            processing order (true ==> suffix order, false ==> prefix
	 *            order),
	 ...
	 *            expression's tree).
	 */
	private void iterate1(BCExpression parent, int chn, boolean suffix,
			ExpressionWalker ew) {
		this.parent = parent;
		this.ChildNo = chn;
		if (!suffix)
	...
	/**
	 * @return expression type (connector), from annot.io.Code interface.
	 */
	...
	/**
	 * @param index -
	 *            index of subespression.
	 * @return subexpression of this expression of given index (with 0 for
	 *         left-most subexpression)
	 */
	 ...
	 * 
	 * @param index -
	 *            index of the subexpression to be set.
	 * @param subExpr -
	 *            new subexpression to be set at <code>index</code> position.
	 */
	...
	/**
	 * Removes all subexpressions and initializes subexpression array.
	 * 
	 * @param n -
	 *            subexpression count (size of the array).
	 */
