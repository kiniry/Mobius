...
/**
 * This is a superclass for all expression that can be OLD. use {@link #isOld()}
 * to check wether this instance is OLD (represents state before this
 * annotation) or not (represents state after this annotation). <br>
 * //XXX what exactly OLD means?
 * 
 * @author tomekb
 */
public abstract class OldExpression extends BCExpression {
	 ...
	 * 
	 * @param isOld -
	 *            wether this expression is OLD or not.
	 */
	 ...
	 * 
	 * @param connector -
	 *            type of expression (from {@link Code} interface).
	 */
	 ...
	 *             represent correct expression of this type.
	 */
	...
	/**
	 * Changes the connector to it's OLD version. //TODO spr. w slowniku:
	 * odpowiednik This method should be updated if any OLD expression is added.
	 * 
	 * @param connector -
	 *            a expression type (from {@link Code} interface).
	 * @return expression type with OLD_ corresponding <code>connector</code>
	 *         expression type, or <b>-1</b> if <code>connector</code> has no
	 *         corresponding OLD_ opcode in {@link Code} interface.
	 */
	...
	/**
	 * Changes the connector to it's non-OLD version. //TODO spr. w slowniku:
	 * odpowiednik This method should be updated if any OLD expression is added.
	 * 
	 * @param connector -
	 *            a expression type (from {@link Code} interface).
	 * @return expression type without OLD_ corresponding <code>connector</code>
	 *         expression type, or <b>-1</b> if <code>connector</code> has no
	 *         corresponding non-OLD_ opcode in {@link Code} interface.
	 */
	...
	/**
	 * This should simply check Expression's type. You can assume, that it's
	 * subexpression is non-OLD.
	 * 
	...
	/**
	 * Check Expression's type and that it's subexpression is non-OLD.
	 * 
	 * @return JavaType of result of this exrpession, or null if it's invalid
	 *         (if one of it's subexpression have wrong type or is invalid).
	 */
