...
/**
 * Abstract class for all prettyPrinters. PrettyPrinters are tools for
 * line-breaking with proper indentation of an expressions. This class contains
 * also some methods that can be useful for there its subclasses.
 * 
 * @author tomekb
 */
public abstract class AbstractPrettyPrinter {
	...
	/**
	 * This method should add line-breaks and indentation to given string to
	 * make it more readable. It should try not produce lines larger than
	 * {@link IDisplayStyle#max_total_line_width}.
	 * 
	 * @param str -
	 *            String representation of an expression to be formatted. Each
	 *            of it subexrpession should be, recursivly, surrounded by block
	 *            marks from Idisplay interface. line-breaks should occure only
	 *            just before or just after a block,
	 * @param spos -
	 *            number of reserved characters in first line (first line of the
	 *            result must be <code>spos</code> characters shorter than
	 *            maximum line width).
	 * @return <code>str</code> with proper line-breaking and indentation.
	 * @see IDisplayStyle
	 */
	 ...
	 * 
	 * @param str -
	 *            String representation of an expression (or its part).
	 ...
	 * @param str -
	 *            String representation of an expression (or its part).
	 * @return length of <code>str</code> (without block marks).
	 */
	 ...
	 * 
	 * @param s -
	 *            formatted (by prettyPrinter) single line String.
	 ...
	 * @param s -
	 *            formatted (by prettyPrinter) single line String.
	 * @return <code>s</code> with '|' character inserted after
	 *         {@value IDisplayStyle#max_total_line_width}'s character.
	 */
	...
	/**
	 * Moves disallowed trailing characters (like '!', '~', '(') to the next
	 * line.
	 * 
	 * @param s -
	 *            formatted (by prettyPrinter) single line String.
	 * @return <code>s</code> with disallowed trailing characters moved to the
	 *         next line.
	 */
			...
			for (;;) {
				...
				for (int j = IDisplayStyle.comment_length; j < line.length(); j++)
					if (line.charAt(j) != ' ') {
						...
						line = l1
								+ prev.substring(prev.length() - suf.length())
								+ l2;
	...
	/**
	 * Correct formatting of annotation or whole bytecode, using filters defined
	 * above. This method should be called just before returning final String
	 * represntation of class' bytecode. Careful of changing this formatting,
	 * code search mechanisms may stop working after some changes!
	 * 
	 * @param str -
	 *            class' bytecode or it part (whole lines).
