...
/**
 * This class represents BMl of an editing bytecode. It's objects can be
 * contructed with {@link BCClass} and current String representation of
 * bytecode. You can add one or more changes (using
 * {@link #addChange(int, int, String)} method), then execute these changes
 * using {@link #performChanges()} method (this will parse all that changes
 * merged to one, single change, updating it's {@link BCClass}) and see that
 * it's correct, than call {@link #resetChanges()} method. If some changes has
 * been parsed outside this class, call {@link #resetChanges()} to assume that
 * {@link BCClass} is up to date. It can parse only BML placed somewhere in
 * bytecode, not the bytecode itself. Bytecode changes won't be updated into
 * {@link BCClass}.
 * 
 * @author tomekb
 */
public class CodeFragment {
	...
	/**
	 * Shows code after preprocessing it by {@link #decorate(String)} method.
	 */
	...
	/**
	 * Code that should be added in merged changes in bytecode.
	 */
	private String toAdd;

	/**
	 * Code that should be removed in merged changes in bytecode.
	 */
	...
	/**
	 * oldCode == prefix + toRemove + suffix; code == prefix + toAdd + suffix;
	 */
	...
	/**
	 * Position of first character on which old and current bytecodes differs.
	 */
	private int begin;

	/**
	 * Position of first unaffected character (in current bytecode).
	 */
	private int end;

	/**
	 * Position of first unaffected character (in original bytecode).
	 */
	private int oldEnd;

	/**
	 * Wether any changes has been added since last {@link #resetChanges()}
	 * call.
	 */
	 ...
	 * 
	 * @param bcc -
	 *            BCClass related with this bytecode,
	 * @param code -
	 *            a String representation of bytecode.
	 */
	...
	/**
	 * Displays an error message and remembers it in errMsg field, as last error
	 * message.
	 * 
	 * @param msg
	 */
	...
	/**
	 * Adds a change (effect of bytecode editing) to this bytecode. A change is
	 * replacing one fragment of this bytecode with another String.
	 * 
	 * @param cfrom -
	 *            position of first modified character,
	 * @param length -
	 *            length of removed String,
	 * @param nc -
	 *            new String added at removed String's position.
	 */
	public void addChange(int cfrom, int length, String nc) {
		...
		if (cto > code.length())
			throw new RuntimeException("invalid position: " + cto
					+ " (length = " + code.length() + ")");
		if (cfrom < 0)
	...
	/**
	 * Translates character position in a String to number of line containing
	 * this character.
	 * 
	 * @param code -
	 *            multi-line String,
	 * @param pos -
	 *            number of character (offset) in <code>code</code>.
	 * @return Number of line in <code>code</code> containing character with
	...
	/**
	 * Translates line number of a String to character number (offset).
	 * 
	 * @param code -
	 *            multi-line String,
	 * @param lnr -
	 *            number of line in <code>code</code>.
	 * @return Offset of first character of <code>lnr</code>'s line in
	 *         <code>code</code>.
	 */
	...
	/**
	 * @return all keywords that can stand at the beginning of an annotation.
	 */
	...
	/**
	 * @return all keywords that can stand at the beginning of an method
	 *         annotation.
	 */
	...
	/**
	 * Searches for given keyword in {@link #getAllAttributeNames()}.
	 * 
	 * @param str -
	 *            a keyword, returned by {@link #getKeyword(String)} method.
	 * @return <b>true</b>, if it is an annotation's keyword, <b>false</b>
	 *         otherwise.
	 */
	...
	/**
	 * Searches for given keyword in {@link #getMethodAttributeNames()}
	 * 
	 * @param str -
	 *            a keyword, returned by {@link #getKeyword(String)} method.
	 * @return <b>true</b>, if it is an method annotation's keyword, <b>false</b>
	 *         otherwise.
	 */
	 ...
	 * @param str -
	 *            a String.
	 * @return wether <code>str</code> is correct String representation of an
	 *         integer or not.
	 */
	 ...
	 * @param line -
	 *            a line of bytecode.
	 * @return <b>"EOM", "EOD" or "EOA"</b> for the same lines,<br>
	 *         <b>"package"</b> for package declaration,<br>
	 *         <b>"class"</b> for class ehader,<br>
	 *         <b>"method"</b> for method header,<br>
	 *         <b>pc number</b> for instruction line (<b>-1</b> if it doesn't
	 *         contain pc number),<br>
	 *         <b>attribute keyword</b> for first line of an annotation,<br>
	 *         <b>"comment_start"</b> for beginning of comment,<br>
	 *         <b>"comment_end"</b> for end of comment, <b>an empty String</b>
	 *         otherwise.
	 */
	public static String getKeyword(String line) {
		...
		if (line.length() < 3)
			return "";
		if (("EOM".equals(line)) || ("EOD".equals(line))
				|| ("EOA".equals(line)))
			return line;
	...
	/**
	 * Returns wether given line contains an annotation's keyword.
	 * 
	 * @param line -
	 *            a line of bytecode.
	 * @return wether given line contains an annotation's keyword.
	 */
	...
	/**
	 * Checks if given line is an annotation's start, or comment border.
	 * 
	 * @param line -
	 *            a line of bytecode.
	 * @return <b>true</b> for first annotation lines and first or last lines
	 *         of comment, <b>false</b> otherwise.
	 */
	@Deprecated
	...
	/**
	 * Adds EOD, EOM and EOA marks to given bytecode. "EOD" line is inserted
	 * after class attributes, but before first method, "EOM" line is inserted
	 * after each method, and "EOA" lines are inserted at the beginning of each
	 * comment and after each annotation. Use {@link #goShowDecoratedCode} flag
	 * to see decorated code while performing changes (at PInfo display level).
	 * 
	 * @param code -
	 *            bytecode to be decorated,
	 * @return <code>code</code> with EOD, EOM and EOA marks inserted, or
	 *         <code>null</code> it <code>code</code> is not correct enough
	 *         to process it.
	 */
	...
	/**
	 * Synchronizes position from original bytecode to bytecode decorated with
	 * {@link #decorate(String)} method.
	 * 
	 * @param oldCode -
	 *            original bytecode,
	 * @param newCode -
	 *            bytecode decorated by {@link #decorate(String)} method,
	 * @param oldPos -
	 *            position (offset) in original bytecode.
	...
	/**
	 * Perform basic syntax checking for given bytecode. Check comment
	 * parenthness and apperance of some keyword (wether they are inside or
	 * outside comment).
	 * 
	 * @param code -
	 *            a String representation of bytecode,
	 * @return <b>true</b> if <code>code</code> is correct enought to attempt
	 *         searching CodePositions in it, <b>false</b> otherwise.
	 */
	private static boolean checkParenthness(String code) {
		...
		for (int l = 0; l < lines.length; l++) {
			...
			if (inComment) {
				if (("mthod".equals(kw)) || ("class".equals(kw))
						|| ("package".equals(kw)) || (isNumber(kw))) {
					MLog
							.putMsg(MLog.PInfo, "invalid keyword in comment: "
									+ kw);
					return false;
				}
				if (isAttributeStr(kw)) {
	...
	/**
	 * Returns {@link CodePosition} representing given offset in given bytecode.
	 * 
	 * @param code -
	 *            a String representation of bytecode,
	 * @param pos -
	 *            offset in <code>code</code>.
	 * @return {@link CodePosition} representing logical position in bytecode,
	 *         related with <code>pos</code> offset in <code>code</code>.
	 */
	 ...
	 *            number of line in <code>code</code>,
	 * @param pos -
	 *            offset in <code>line</code>.
	 * @return {@link CodePosition} representing logical position in bytecode,
	 *         related with given offset in <code>code</code>.
	 */
	...
	/**
	 * Commits BML changes in bytecode into BCClass. First, checks that current
	 * bytecode is correct, then parse it, affecting as few elements of BCClass
	 * as it can. Parsing whole class can be uneffective (slow), so it tries to
	 * skip unaffected BML annotations, comments and methods. It can parse only
	 * BML annotations, not the bytecode itself (bytecode-level (BCEL's)
	...
	public void performChanges() {
		...
		boolean mma = (cp_start.getMet_nr() != cp_old.getMet_nr())
				...
				|| (cp_start.isInClassAttribute() != cp_old
						.isInClassAttribute())
				|| (cp_start.isInClassAttribute() != cp_new
						.isInClassAttribute());
		...
		for (int l = 0; l < lines.length; l++) {
			...
			affm = (cp_start.getMet_nr() <= mnr)
					&& ((cp_old.getMet_nr() >= mnr)
							|| (cp_new.getMet_nr() >= mnr) || (cp_old
							.getMet_cnt() != cp_new.getMet_cnt()));
			if ("EOM".equals(kw)) {
				if (!affm)
			...
			// instructions
			affi = mma
					|| (cp_start.getInstr_nr() <= inr)
					&& ((cp_old.getInstr_nr() >= inr)
							|| (cp_new.getInstr_nr() >= inr) || (cp_old
							.getInstr_cnt() != cp_new.getInstr_cnt()));
			if (isNumber(kw)) {
				lines[l] = "[instruction]";
				if (!affi) {
					// if (mspec) {
			...
			if ("EOA".equals(kw)) {
				...
				if ((anr >= 0) && (!affa)) {
					if (mspec) {
						// XXX this should be updated while adding new nethod
						// attribute.
						if (IDisplayStyle._requires.equals(akw)) {
							lines[l] = "\\method specification unaffected";
						} else {
							throw new RuntimeException(
									"invalid method attribute not detected: "
											+ kw);
						}
					} else {
				...
				affa = (cp_start.getAnnot_nr() <= anr)
						&& ((cp_old.getAnnot_nr() >= anr)
								|| (cp_new.getAnnot_nr() >= anr) || (cp_old
								.getAnnot_cnt() != cp_new.getAnnot_cnt()));
				continue;
			}
			if (!affa) {
				lines[l] = null;
		...
		shortCode = shortCode.replaceAll(Parsing
				.escape(IDisplayStyle.comment_end)
				+ " *\n", "\n");
	...
	/**
	 * Assumes that bytecode has been parsed succesfuly. It should be called
	 * after each {@link #performChanges()} call, if bytecode is correct.
	 * Calling it for incorrect bytecode may result of ignoring some errors in
	 * next parsing attempt. This method has been separated from
	 * {@link #performChanges()} method only for test pusposes, to show
	 * {@link CodeFragment}'s state just after parsing it.
	 */
	...
	/**
	 * Modifies current bytecode, replacing given fragment with another String
	 * and updating {@link BCClass}. Either use this or seuqntly:<br> -
	 * {@link #addChange(int, int, String)} (one or more times (for merged
	 * changes)),<br> - {@link #performChanges()},<br> -
	 * {@link #resetChanges()}.
	 * 
	 * @param cfrom -
	 *            position of first modified character,
	...
	/**
	 * @return a hash code for this editing bytecode (the same result if (but
	...
	/**
	 * @return wether current bytecode is correct. It can ignore errors that are
	 *         far enought from edited fragment (eg. if they were there at the
	 *         beginnin, or before last {@link #resetChanges()} call.
	 */
	...
	/**
	 * @return last error message. Currenlty error messages tells only if code
	 *         were parsed by ANTLR, or was so incorrect that was rejected
	 *         before passing it to ANTLR.
	 */
	...
	/**
	 * Searches nearest bytecode (not BML) line above given one in given
	 * bytecode. For use in code position's synchronization in editors.
	 * 
	 * @param code -
	 *            a bytecode with BML annotations,
	 * @param lnr -
	 *            number of line in <code>code</code>.
	 * @return number of nearest line in <code>code</code> above
	 *         <code>lnr</code>'s that is not a BML annotation's line.
	 */
	...
	/**
	 * Searches nearest bytecode (not BML) line below given one in given
	 * bytecode. For use in code position's synchronization in editors.
	 * 
	 * @param code -
	 *            a bytecode with BML annotations,
	 * @param lnr -
	 *            number of line in <code>code</code>.
	 * @return number of nearest line in <code>code</code> below
	 *         <code>lnr</code>'s that is not a BML annotation's line.
	 */
