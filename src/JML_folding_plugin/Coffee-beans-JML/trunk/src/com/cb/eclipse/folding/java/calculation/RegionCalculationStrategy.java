...
public interface RegionCalculationStrategy {
	...
	// @phosphorus
	/**
	 * Sets the document reference so that it is possible to access
	 * the document directly. This is needed to determine if comments
	 * are JML annotations or not as the editor treats the two as the same.
	 * 
	 * @param doc the document that we want to set
	 */
	//void setDocument(IDocument doc);
	
	/**
	 * see setDocument(IDocument doc)
	 * @return the document that is being worked on.
	 */
