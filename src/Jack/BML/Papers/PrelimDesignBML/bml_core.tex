...
\jmlKey{length(\(a\))}, denoting the length of array \(a\). Since the 
source code expression \jmlKey{\(a\).length} is compiled into a
special bytecode instruction \jmlKey{arraylength}, we also need a
...
\item loop variants (using the \jmlKey{decreasing} keyword).
\end{itemize}
...
\jmlKey{loop\unsc modifies}, which is specified together with the loop
invariant. This clause specifies which variables
\emph{may} be modified by the loop (as an \jmlKey{assignable} clause does
for a method). This \jmlKey{loop-modifies} clause allows to write
...
%\jmlKey{\#}\emph{natural}} &\% reference in the constant pool \\
%&\(\mid\)\jmlKey{super} \(\mid\) \jmlKey{this}\\
%\multicolumn{2}{l}{\emph{store-ref-name-suffix} ::= 
%\jmlKey{(}\emph{store-ref-expression}\jmlKey{)}}\\
%&\(\mid\) \jmlKey{[}\emph{spec-array-ref-expr}\jmlKey{]}
\end{tabular}
...
\jmlKey{\#}) or to local registers. The local register \jmlKey{lv[0]}
of a non-static method always contains the implicit argument
\jmlKey{this}, the other registers contain the parameters and the
...
(\jmlKey{cntr}), and to refer to an element on the stack
(\jmlKey{st(\(e\))}, where \(e\) is some arithmetic expression).
...
%\jmlKey{obj.f}, where \jmlKey{obj} is the first parameter of a
%method, and \jmlKey{f} is a field of this object. This becomes
%\jmlKey{\#\(n\)(lv[1])} in BML, where \(n\) is the index in the
%constant pool to the field constant reference denoting the field
%\jmlKey{f}, while \jmlKey{lv[1]} is the register in the local variable
%array in which the parameter \jmlKey{obj} is stored.  Therefore the
...
\caption{Bytecode + BML specification for method \jmlKey{produce\unsc bill} in class \jmlKey{Bill}}\label{FigBMLSpec}
\end{figure}

To show a typical BML specification, Figure~\ref{FigBMLSpec} presents
the BML version of the specification of method \jmlKey{produce\_bill}
of the JML example in Figure~\ref{FigJMLSpec}. Notice that the field
\jmlKey{sum} has been assigned the number 24 in the constant
pool, and that it is always explicitly qualified with \jmlKey{lv[0]}
(denoting \jmlKey{this}). Further, \jmlKey{lv[1]} denotes the
parameter \jmlKey{n}, while \jmlKey{lv[2]} denotes the local variable
\jmlKey{i}.
...
This expression is not qualified with \jmlKey{lv[0]}, as it is
...
%private, but \jmlKey{spec\unsc public} (i.e.~publicly visible
%in specifications) field
%\jmlKey{list}, which is an array of objects. The class invariant says
%that \jmlKey{list} is not null, and in addition, its elements are
%never null.  Further, the class contains a method \jmlKey{replace},
%which checks if its first parameter \jmlKey{obj1} occurs in
%\jmlKey{list}, and if this is the case, replaces it (once) by its
%second parameter \jmlKey{obj2}. 
...
%\end{verbatim}
%}
%\caption{JML specification for class \jmlKey{ListArray}}\label{FigJMLSpec}
%\end{figure}
...
expressions $e_1$ and $e_ 2$ are of type \jmlKey{java.lang.Class}
...
These confirmedness checks for BML are best implemented as an
...
%variables can still be of type \jmlKey{boolean} --- this information is
%used by the BCV). Thus, to make sure that expressions such as
%\jmlKey{\bsl result == \bsl exists i. i >= 0} are correctly evaluated,
...
\paragraph{Statement annotations}
...
For mapping loop frame conditions, we use again the fact that in the
...
%\paragraph{Example}
%\label{par:embedding:running}
%Consider again the BML specification of the class \texttt{Bill} in
%Figure~\ref{FigBMLSpec}. We have already shown above how the different
%predicates can be translated into the deep expression language. Let us
%use \(\gamma_{\mbox{\tiny\textsc{PRE}}}\), 
%\(\gamma_{\mbox{\tiny\textsc{POST}}}\) and
%\(\gamma_{\mbox{\tiny\textsc{INV}}}\) 
%%for expressions \eqref{RunningExamplePre},
%%\eqref{RunningExamplePost} and \eqref{RunningExampleInv},
%respectively. Let us further assume that \(\mathcal{I}\) is the deep
%expression encoding the class invariant \texttt{\#24 >= 0},
%\emph{i.e.}\
%\[
%\mathcal{I} = \mathit{LookUpHeapBML This (FieldName Bill.sum)} \leq 0.
%\]
%If we map the BML specification into the \mobius base logic, this will
%result in the following judgement:
%\[
%\JudgeF{\ppt}
%       {\mathit{MPreTag(\mathcal{I}) \wedge \gamma_{%
%                                               \mbox{\tiny\textsc{PRE}}}}}
%       {\mathit{MPostTag(\mathcal{I}) \wedge \gamma_{%
%                                               \mbox{\tiny\textsc{POST}}}}}
%       {\mathsf{true}}.
%\]
%Finally, the local annotation table \(\Anno\) will contain a single entry:
%from instruction 22 (loop entry point) to \(\gamma_{\mbox{\tiny\textsc{INV}}}\).
