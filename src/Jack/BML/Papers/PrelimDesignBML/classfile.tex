...
To compact the specifications even more, one can use a desugaring 
as defined for JML~\cite{RaghavanL00}. This allows to reduce the
number of specification clauses, and also to unify the different kinds
of behaviour specifications. 

Thus, to ensure JVM compatibility, the different specification
constructs are stored in special tables, and where necessary these
attributes contain information about the index of the instruction that
they specify. For instance, the attribute that stores BML loop
invariants contains the invariant as well as the index of the entry
point instruction of the loop.

For each class, we add the following global user-specific attributes:
\begin{itemize}
