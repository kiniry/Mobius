...
& \\ 
  \MethodSpec & ::=  \begin{tabular}{l}   
                          \requires \ \formulaBc;\\
                          \specCases; \\
                    \end{tabular}\\
& \\
\specCases & ::= \specCase \\     
                    & $\mid$  \specCase \ \also \ \specCases \\

 & \\
  \specCase & ::=    \{$\vert$ \\
                    & \phantom{aaa} \begin{tabular}{l}  
                                 \requires  \ \formulaBc; \\
                                 \modifies  \  \modifiesLocs;  \\
                                 \ensures \ \formulaBc; \\
                                 \exsuresList\\
...
& \\
\modifiesLocs & ::=  [ ] $\mid$ \modifiesLoc, \modifiesLocs \\
 & \\
  \modifiesLoc & ::=  \fieldAccess{$\expression$}{\FieldConstRef } \\
                    & $\mid$ \locVar{i} \\ 
 ...
 & \\
 \specIndex & ::= \all  $ \mid $   $\expression_1 $ \ldots  $\expression_2 $  $\mid $    $\expression $ \\            
%\end{array}                
%$$
...
Note that most of them have an identical counterpart in JML
 ...
 % Thus, we will concentrate more on
 %  the specific features of BML and will just briefly comment
...
Because intermediate calculations are done by using the stack, often we will need stack expressions in order to characterize the states before and after an instruction
...
% The first specification case (the first bunch of specification enclosed in $\{|$ $ |\}$  ) corresponds to the \jmlKey{normal\_behavior} specification case in the code from Fig. \ref{bml:heavySp}.
 ...
 % for any exceptional termination.  The second specification case in Fig.\ref{bml:heavySpBML}  corresponds to the \jmlKey{exceptional\_behavior} case of the source code
