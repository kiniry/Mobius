...
Note that most of them are represented with their notation in JML.
\subsubsection{BML expressions}
Most of the expressions supported in BML have their counterpart in JML. However, few 
of the expressions generated by the nonterminal $ \expression$ \  do not have analogs in JML. 
As we will see hereafter, BML allows
...
The nonterminal $\expression$ generates expressions while the nonterminal  $\typeExp$ gives the grammar for 
type expressions.
...
\begin{itemize}
      ...
      \item  $\locVar{i}$ a local variable in the array of local variables of a method at index $i$.
             ...
             is the list containing both the formal parameters of the method and the variables declared locally in the method.
             
     
      \item  $\fieldAccess{ \expression}{\ident}$ stands for accessing the field which is at index $ \ident $ 
              in the class constant pool of the class 
      ...
      \item $\old{\expression}$  denotes the value of $\expression$ in the initial state of a method.
            This expression is makes sense in the postcondition of a
