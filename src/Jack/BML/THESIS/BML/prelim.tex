...
suggest every of them specifies a specific normal or exceptional behavior of a method.  (see \ref{PD06LBR}). The keyword 
 \jmlKey{normal\_behavior} introduces a precondition which if it holds in the prestate of the method execution then
the method will terminate normally and the postcondition introduced by the keyword \jmlKey{requires} will hold.
...
An example for a heavy weight specification is given in Fig. \ref{bml:heavySp}. \todo{continue here}
