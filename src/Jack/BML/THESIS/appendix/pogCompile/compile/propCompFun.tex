...
\begin{compPropApp}[Compilation of statements and expressions]\label{compile:prop:compProp1}
    ...
    the compiler will produce a list of bytecode instruction $\compileLabel{s}{\ExprStmt}{e}$ such that: 
           $$ \begin{array}{l} 
			      ...
			      ( \ins{i} \execRel \ins{k}) \wedge 
			      \neg (\ins{k} \in   \compileLabel{s}{\ExprStmt}{e}  ) \wedge
...
and their  compilations  are $\compileLabel{s}{\ExprStmt}{e}$ and  $\compileLabel{s'}{\ExprStmt'}{e'}$  then the following holds:
$$
                        (  j \execRel k \wedge  j \in \compileLabel{s}{\ExprStmt}{e} \wedge
		        \neg (j \in \compileLabel{s'}{\ExprStmt'}{e'} )   \wedge 
		        k \in \compileLabel{s'}{\ExprStmt'}{e'}  
		        )  \Rightarrow   k = s'  $$ 

\end{compPropApp}
The proof is by induction over the structure of statements and expressions  and uses  Lemma \ref{compile:prop:compProp1}.
