...
%\end{frameit}
\end{figure}
As we can see from the figure, the architecture relies on:
\begin{itemize} 
   ...
   \item an algorithm which  generates logical (verification) conditions  from the annotations and the program.
	 ...
	 that the implementation respects the requirements.
...
Examples for mobile code scenarios are the downloading from the web and execution of software components. The latter could be
from user specific applications to patches for the operating system.
 In those cases, the client system is compromised as far as there is no guarantee that the software component respects its internal requirements. Client requirements can 
be that the code must be  well typed or that it accesses the memory in a save way. Those properties are low level in the sense that they guarantee
that the software component does not violate the system execution. Client requirements can be also of  higher level, for instance that the component has a certain functional
 properties. For instance, this can be in cases when an updated implementation of an interface is downloaded on the  client computer system and 
patches an old implementation of the interface. In this cases, the client system needs guarantees that the unknown software component respects the specification 
of the interface. 
...
Returning back to mobile code scenarios, it is more suitable to perform the verification  directly (in the case of Java) bytecode. 
First, because the bytecode is normally not accompanied by the corresponding source code and second, even if it were this would require 
trust in the compiler. Thus, the client should perform \textit{ verification on bytecode level}. This would mean that  the client should 
have the infrastructure shown above in Fig. \ref{intro:srcVerif}.but this time for bytecode.
...
Providing mechanisms for bytecode verification gives the client opportunity to check if the application respects the requirements of the client system.
...
In the traditional PCC architecture, untrusted code is accompanied by a proof which certifies the client  requirements. In Fig. \ref{intro:PCC}, we show  the basic components of such framework and the relation between them. The code producer uses the so called certifying compiler. The certifying compiler consists of a compiler from source code
to executable (or interpreted bytecode) which generates also annotations for the executable  and a certifier. The certifier assembles the verification condition generator
over the bytecode and an automatic decision procedure which will generate the proof for the verification condition generator. The producer sends the 
...
The complexity of type checking algorithms is small and thus, do not compromise  the client system.
\begin{figure}[ht!]
%\begin{frameit}
\begin{center}
%\includegraphics{bc.eps}
...
This limitation is not satisfactory, because there are potentially scenarios where the client requires that the application respects more complex properties.      
For instance, the client may have functional requirements over the unknown code. An example is  when an update version  of an interface   implementation
...
on the producer site interactive. We propose that the producer site \textit{generates the certificate interactively over the source code}. The advantage of 
...
However, there are several points which need more attention.
...
This is the last ingredient of the schemata shown in Fig. \ref{intro:PPO}.


We actually propose a new framework for mobile computing shown in Fig. \ref{intro:PPO}. 
...
against potentially non trivial functional and security properties. The framework is tailored to Java bytecode.
 ...
 sufficient mechanisms for their guarantee, e.g. preserving of confidentiality or limitted use of computational resources. Moreover,
 ...
 however no mechanisms are provided by the Java system that they are not malicious.
