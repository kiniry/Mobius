...
function $\excPostSrc$  and  returns the    formula 

$\wpSrcExpr{\expressionSrc}{\psi}{\excPostSrc}{ v }$ which is the \wpName \ precondition of expression $\expressionSrc$ 
if the its evaluation is stored in the special variable expression $v$.  
...
construct is that it creates a new instance of the corresponding class (denoted  in the figure with \class) and
then initializes it by invoking the class constructor \Constructor{\class}.
...
terminates execution. In the case for the class constructor which in Java and in our modelization are also methods 
and thus a constructor is supplied with  
 precondition $\Constructor{\class}.\preSrc$  and a postcondition $\Constructor{\class}.\normalPostSrc$.
Thus, the  rule for instance creation states that in the state where $\Constructor{\class}$  is invoked 
 its precondition  $\Constructor{\class}.\preSrc $ holds and  in the state in which $\Constructor{\class}$ 
 terminates execution its postcondition $\Constructor{\class}.\normalPostSrc$ implies the postcondition $\normalPostSrc$  of 
the expression. Note that the implication is quantified over the newly created  instance which did not existed in the prestate
 of the instance creation expression and which is different from \Mynull. The fact that the instance is new is expressed  via the  predicate ($\neg \ \instances(\freshVar ) $) whose
semantics is that \freshVar{} does not belong to the heap in the prestate of a method. 
