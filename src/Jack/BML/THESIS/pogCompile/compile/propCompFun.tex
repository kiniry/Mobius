...
The compilation  $\compileLabel{s}{\stmt}{e}$ of a statement $\stmt$ may contain an instruction $\ins{k}$ and \ins{j}  which are respectively 
...
Consider in Fig. \ref{pogEquiv:compile:prop:exc} the bytecode version of the program. The last  instruction in the compilation of the \lstinline!if! statement is the 
...
The next property states that a try catch statement $ \try \ \{ \stmt_1 \} $ $ \catch \ (\excType \ \var )$ $\{ \stmt_2 \}  $  is such that
