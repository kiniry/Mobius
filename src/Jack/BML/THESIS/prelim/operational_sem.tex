...
\section{Bytecode language and its operational semantics} \label{opSem}
 ...
 In particular, it supports object manipulation and creation, method invocation, as well as exception throwing and handling. In fig. \ref{opSem:bclang}, we give the list of instructions that constitute our bytecode language\footnote{The instruction \arithOp{} stands for any arithmetic instruction in the list  \instr{add}, \instr{sub}, \instr{mult}, 
...
The following definition characterizes the executions that terminate. 
 ...
 \begin{transClosStateTrans1}[Termination  of method execution] \label{stateTransClos}
  ...
  % $\config{ \heap}{\counterOnly}{\stackOnly}{\locVarOnly}{k}$ 
...
\end{figure}
 ...
 The stack counter is decremented and  the resulting  value on the stack top \stackOnlyParam{\counterOnly - 1} \ \op \ \stackOnlyParam{\counterOnly}
 is pushed on the stack  top  \stackOnlyParam{\counterOnly - 1}. Note that our formalization does not take into consideration overflow of arithmetic instructions, i.e.
 we assume that we can manipulate any unbounded integers.  
 We may remark that there are two rules  for the arithmetic instructions  \instr{div} and \instr{rem}. This is because these instructions
...
\frac{ 
	   ...
	   \stackOnly' = \update{\stackOnly} {\counterOnly-1}{\stackOnlyParam{\counterOnly} \ \op \ \stackOnlyParam{\counterOnly-1}} \\
	   \pc' = \pc + 1
	   \end{array}}
		   {\method \vdash  \arithOp :\config{\heap}{\counterOnly}{\stackOnly}{\locVarOnly}{\pc} 
		          \stateTrans 
                          \config{\heap}{\counterOnly'}{\stackOnly' }{\locVarOnly}{\pc'} } %}
 \\ \\ 
%{\scriptsize 
...
%{\scriptsize
 \frac{ \begin{array}{l}
	               ...
	               \stackOnlyParam{\counterOnly - 1} \neq \Mynull   \\
                       % \stackOnlyParam{\counter} \in  \Myint \vee \stackOnlyParam{\counterOnly - 1}  =\Mynull  \vee  \stackOnlyParam{\counterOnly }  \\
		       %\left\{\begin{array}{l}
...
\caption{\sc Operational semantics for field manipulation }
\label{opSem:obj:manip1}
\end{frameit}
\end{figure}
...
and the reference value contained in \stackOnlyParam{\counterOnly - 1}. If the reference
  \stackOnlyParam{\counterOnly - 1} is not \Mynull,  
...
the function  standing for the	\fieldd{}   is updated with the value\stackOnlyParam{\counterOnly} for the reference  \stackOnlyParam{\counterOnly - 1}  and
...
\begin{frameit}
 ...
 \frac{ \begin{array}{l}  
	             \stackOnlyParam{\counterOnly - 2} \neq \Mynull  \\
                    % \stackOnlyParam{\counterOnly - 1}  \in \heap.\heapLocs \vee \stackOnlyParam{\counterOnly - 1}  =\Mynull  \vee  \stackOnlyParam{\counterOnly - 1}  \in \Myint \\
...
\frac{ \begin{array}{l}        
 ...
 % {\scriptsize 
\frac{ \begin{array}{l}        %\stackOnlyParam{\counterOnly - 1}  \in \heap.\heapLocs \\
                              \stackOnlyParam{\counterOnly - 1} \neq \Mynull  \ \\
...
\frac{ \begin{array}{l}         \\
  ...
  %   {\scriptsize
 \frac{ \begin{array}{l}           % \stackOnlyParam{\counterOnly }  \in \heap.\heapLocs \\
			            \stackOnlyParam{ \counterOnly } \neq \Mynull  \\
...
\caption{\sc Operational semantics for array manipulation }
\label{opSem:obj:manip2}
\end{frameit}
\end{figure}
...
The value in the stack top element \ is stored in the component at index \stackOnlyParam{\counterOnly - 1} \  of the array  in  \stackOnlyParam{\counterOnly - 2}.
If the array reference \stackOnlyParam{\counterOnly - 2} \ is \Mynull{} a \NullPointerExc{} is thrown. If  \stackOnlyParam{\counterOnly - 1}   is not in the bounds of the array 
...
%	{\scriptsize
	\frac{ \begin{array}{l}  \neg ( \subtype{ \heap.\heapTypeOf( \stackOnlyParam{\counterOnly} ) }{\clazz}) \vee
                            \stackOnlyParam{\counterOnly} = \Mynull  \\
			       %   \begin{array}{l}
...
\\ \\ 
 ...
 %  {\scriptsize
 \frac{\begin{array}{l}        \subtype{\heap.\heapTypeOf( \stackOnlyParam{\counterOnly})}{\clazz }   \vee
                               \stackOnlyParam{\counterOnly} = \Mynull  \\
			      \pc' =  \pc  +1 
		 \end{array}}
...
\begin{frameit}
 ...
 \frac{ 
	             \begin{array}{l}	 
                          % \stackOnlyParam{\counterOnly}  \in \heap.\heapLocs \\
		           \stackOnlyParam{\counterOnly} \neq \Mynull   \\
...
\end{figure}
       ...
       Finally, in Fig.\ \ref{opSem:obj:method}, we can see the semantics of method invocation. 
       ...
       Once  the proper method method \mbox{\rm n} is found it is invoked.
        % The first element on the stack top is required to be a valid reference in the heap and the 
 % 	 arguments passed to the invoked method, i.e. the following $\mbox{\rm n}.\numArgs$ elements on the stack top are required to be 
	 %  either a valid reference in the heap or of an integer type (the predicate \inHeap).
      Next, the first top
	  $\mbox{\rm n}.\numArgs$ elements in the operand stack \stackOnly \ are popped from the operand stack. If 
         \stackOnlyParam{ \counterOnly - \mbox{\rm n} .\numArgs } is not \Mynull, the  method   \mbox{\rm n}  is executed on the object   \stackOnlyParam{ \counterOnly - \mbox{\rm n}.\numArgs } 
	 and where the first \mbox{\rm n}.\numArgs + 1 elements of the list of its of local variables is initialized with 
         \stackOnlyParam{ \counterOnly - \mbox{\rm n} .\numArgs } \ldots \stackOnlyParam{\counterOnly}. In case that the execution of method \mbox{\rm n}
	 ...
	 a handler that can handle the exception. In case the object  \stackOnlyParam{ \counterOnly - \mbox{\rm n}.\numArgs } \  on which the  method \mbox{\rm n} must be 
...
\end{array}$$
\caption{\sc Operational semantics for method invocation}
% The predicate $\inHeap(0 \ldots \mbox{\rm n}.\numArgs  )$ is an abbreviation for 
%  $ \stackOnlyParam{ \counterOnly } \in \heap.\heapLocs \wedge \forall i, 1 \le i \le \mbox{\rm n}.\numArgs \Rightarrow \stackOnlyParam{ \counterOnly - \mbox{\rm n}.\numArgs } \in \heap.\heapLocs \vee \stackOnlyParam{ \counterOnly - \mbox{\rm n}.\numArgs } = \Mynull \vee \stackOnlyParam{ \counterOnly - \mbox{\rm n}.\numArgs } \in Myint  $}
