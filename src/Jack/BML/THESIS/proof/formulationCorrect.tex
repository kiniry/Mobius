...
$$ \begin{array}{l}
              \frac{ i \neq \invoke \ \ \methodd \vdash  i: s_1 \stateTransWeight{k} s_2 \  \level{k} > 0}{ \methodd \vdash s_1 \stateTransWeight{k} s_2 }
\end{array}$$
...
A method respects its specification if every terminating execution is such that if in its initial state
 the method precondition holds  then in its final state the method postcondition holds. We also have to define the method correctness at execution at level  \level{k} which follows. 
...
and the exceptional postcondition function \methodd.\excPostSpec, we say that \methodd{} respects its specification at level \level{k} if 
...
generated for all methods in a program are valid then we can conclude that the program is correct. 
%are valid 
