...
This definition is rather intuitive and means that if a method \methodd{}
 has a maximal call depth \level{k+1} any method \mbox{\rm meth}  called in its body will have a maximal call depth \level{k}. 
If a method execution has a maximal depth call \level{1} this means that the method execution does not perform any method calls.
Thus an execution labeled with \level{0} does not exist.
...
\begin{defCorrect}[A method respects its specification at level k] \label{defCorrectLevel}
...
Next, we give a definition for program correctness. 

\begin{defCorrect}[A program is correct at level \level{k}]\label{defCorrectProgramLevel}
...
\begin{defCorrect}[A program is correct]\label{defCorrectProgram}
