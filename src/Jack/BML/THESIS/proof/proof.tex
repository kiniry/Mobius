...
to the operational semantics described in Section \ref{opSem}.
Remind that, we look only at
 partial correctness, i.e. our calculus works under the assumption that 
a progam terminates. 
...
\begin{enumerate} 
	...
	\item   $\ins{\pc_n } = \load \ i $ 
	             \todo{Easy: say what are other cases are similar to this one}
...
\mbox{\rm \comment{by definition of  the wp function in section \ref{wpRules} of the previous chapter} } \\
...
\begin{lemma3}[\fwpi \ precondition for method entry point holds initially ]\label{lemma3}
...
\begin{enumerate}
    \item\textit{ $s_{0} \stateTrans s_{1}$. By initial hypothesis we have that  $\interp{ \wpi{\methodd.\body[0] } {\methodd}{}}{ s_{0}}$
             ...
             $s_0$  holds}   
    \item \textit{Induction hypothesis:   $s_{0} \stateTrans^{n - 1} s_{n - 1}$ and \\
...
\begin{lemma2}[\fwpi \ precondition for method entry point holds initially]\label{lemma2}

Assume we have a method \methodd \ with normal postcondition  $\methodd.\normalPost$ and exception function $\methodd.\excPostSpec$. 

%Assume that every execution of the method \methodd \ terminates.
 Assume that  execution of method \methodd{} starts in state $_{0}$ 
and $\interp{ \wpi{\methodd.\body[0] } {\methodd}{0}}{ s_{0}}$
Then if the method \methodd{} terminates, i.e. there exists a  state $s_n$,  $\methodd : s_0  \stateTransTerm s_n   $ 
...
\textit{Let $ \ s_{0} \stateTrans^{*} s_n $ and $\methodd.\body[\pc_n]$ is a \return \ or an instruction that throws a  not handled exception.
...
$$ \numConclusion{1} \ \validFormula{\methodd.\pre \Rightarrow \wpi{\method.\body[0]}{\methodd}{}} $$
 then \methodd \ is correct in the sense of the Def. \ref{defCorrect}. 
\end{vcGenCorrect}
Proof:
\textit{ 
As we want to prove that method \methodd{} is correct w.r.t. its specification we have
as hypothesis that the method precondition holds in the  the initial state $s_0$ 
of the method execution, or $\interp{ \methodd.\pre }{ s_{0}}$.
From the latter and \numConclusion{1}, we get that the weakest precondition of the 
entry point holds in the initial state, i.e.  $\interp{ \wpi{\methodd.\body[0] } {\methodd}{}}{ s_{0}}$. 
We can apply then Lemma \ref{lemma2} and we get that the respective method postcondition holds in the final state 
of the method \methodd. We can conclude then that \methodd \ respects  its specification in the sense of Def. \ref{defCorrect}.
 }\\
\Qed
