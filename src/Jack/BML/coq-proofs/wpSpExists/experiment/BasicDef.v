...
Definition eq_AuxVar x y := Zeq_bool x y.



Definition auxState : Set := auxVar -> value.


Definition state : Set := var -> auxState -> value.

Definition update (s:state) (auxS : auxState  ) (x:var) (v:value) :=
 fun y => if eq_var x y then v else ( s  y auxS ). 
...
Fixpoint eval_expr (s:state) ( sAux : auxState) (e:expr) {struct e} : value :=
 match e with
 | Econst n      => n
 | Evar x          => s x sAux   
 | EbinOp op e1 e2 => eval_binop op (eval_expr s sAux e1) (eval_expr s sAux e2)
 | EunOp  op e1    => eval_unop  op (eval_expr s sAux e1)
 end.
