...
The overall objective is to allow a client to trust a code produced by an untrusted code producer. Our approach is especially suitable
 ...
 The framework is presented in Fig.~\ref{architecture}; note that certificates are not yet implemented
...
\caption{\sc The overall architecture for annotating and verifying code}
\label{architecture}
\end{center}
\end{figure}
%\clearpage
...
discharging proofs which represent the program safety certificate. Then every Java file of the 
...
The producer generates the program certificate from the source code and sends it to the client along with the bytecode.
