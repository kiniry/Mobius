...
\SpecCase ::= \jmlStmt{requires} \ \predicate;\\ 
	\Myspace \Myspace \Myspace \Myspace  \Myspace \jmlStmt{modifies} \ list(\expression);\\
	%\Myspace \jmlStmt{decreases} \ \expression;\\
    \Myspace \Myspace \Myspace \Myspace \Myspace \jmlStmt{ensures} \ \predicate;\\
    \Myspace \Myspace \Myspace \Myspace \Myspace \jmlStmt{exsures} \ (\texttt{ExceptionClass}) \ \predicate;\\
                     \\
...
\loopSpec ::= \jmlStmt{pc\_ index} \ int;\\
	    \Myspace \Myspace \Myspace \Myspace \Myspace \jmlStmt{loop\_modifies} \ list(\expression);\\
       \Myspace\Myspace \Myspace \Myspace  \Myspace \jmlStmt{loop\_invariant}  \ \predicate; \\
	   \Myspace \Myspace \Myspace \Myspace \Myspace \jmlStmt{loop\_decreases} \ \expression; \\
	 \\								
\assert   ::= \jmlStmt{pc\_ index} \ int;\\
		\Myspace \Myspace \Myspace \Myspace \jmlStmt{assert} \ \predicate; \\
	\\

      \predicate ::= \true \ \mid \ \false \\
      \Myspace  \Myspace \mid   \expression \ predSymbol \ \expression \\
       \Myspace \Myspace \mid   \predicate \wedge \predicate \mid   \predicate \vee \predicate \mid   \predicate \Rightarrow \predicate \\
  ...
  \expression ::= \ArithExpr  \mid  \ \register{i} \\
	  \Myspace \Myspace \mid  \reference \mid \ \intLiteral \\	
  	  \Myspace \Myspace  \mid  \ \fieldAccess{\expression} \  \mid \ \arrayAccess{\expression} {\expression}  \\
