...
\assert   ::= \\
  ...
  \expression ::= \\  
      \Myspace  \ArithExpr  \mid  \ \register{i}  \  \mid  \ \reference \\
  	  ...
  	  \Myspace \mid \typeof{\expression}  \ \mid \ \Mynull \ \mid \  \this \ldots
\end{array}$$
...
Thus for example the loop invariant specified in JML for the method \texttt{replace} at ~\fig{replaceSrc} is :
$$
\begin{array}{l}
...
\end{array}
$$
From the example you can see that local variables and  fields are respectively linked to the index of the register table for the method and to the corresponding index of the constant pool table. 
