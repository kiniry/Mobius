...
exceptions, references, method calls and subroutines.  
 ...
 inserted in the class file format in newly defined attributes, thus making not only the code mobile but also its specification
and allowing the Java bytecode benefit from the source specification. These class
 file extensions do not affect the JVM performance. We have implementations of a verification condition generator based on the weakest precondition calculus and of the JML specification compiler. Both are integrated in the Java Applet Correctness Kit (JACK)~\cite{BRL-JACK}. 
