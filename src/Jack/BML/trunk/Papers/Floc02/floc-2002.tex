...
\>\>\>{\it AST temp4$\_$MOD = (AST)astfactoryMOD.dupTree(returnAST) ;} \\
\>\>\>$\mathtt{match(RPAREN)\ ; \cdots statement(in\_model\_prog)\ ;}$ \\
\>\>\>{\it //code added for modifiable checker} \\
\>\>\>{\it AST temp5$\_$MOD = (AST)astfactoryMOD.dupTree(returnAST) ;} \\
\>\>\>{\it Modifiable.$\_$WHILE(temp4$\_$MOD,temp5$\_$MOD, currentContext.getY(), contextStack) ;} \\
\>$\mathtt{\}}$ 
\end{tabbing}
...
\>\>$\mathtt{tempRes\ =\ \_in\_PRIME(astfactoryMOD.dupTree(e.getFirstChild(\ )),Y,contexts)\ \&\&}$  \\ %{\it //e1 $\underline\in$ Y} \\
\>\>\>$\mathtt{\_modPE(astfactoryMOD.dupTree(e.getFirstChild(\ )),Y,contexts)\ \&\&}$  \\ %{\it //e1 modPE Y} \\
\>\>\>$\mathtt{\_modEXP(astfactoryMOD.dupTree(e.getFirstChild(\ ).getNextSibling(\ )),Y,contexts)\ ;}$ \\% {\it //e2 modEXP Y} \\
\>\>$\mathtt{break\ ;}$ \\
\>\>\vdots \\
\>$\mathtt{\}}$ \\
