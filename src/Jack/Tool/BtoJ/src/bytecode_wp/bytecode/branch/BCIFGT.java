...
public class BCIFGT extends BCConditionalBranch {
	...
	public Formula wp(IJml2bConfiguration config,
		...
		Formula stackTop_not_grt_0 = new Predicate2Ar(new Stack(
				Expression.COUNTER), new NumberLiteral(0),
				PredicateSymbol.LESSEQ);
		Formula not_grt_branch = (Formula) _normal_Postcondition.substitute(
				Expression.COUNTER, Expression.getCOUNTER_MINUS_1());
		wp = Formula.getFormula(stackTop_not_grt_0, not_grt_branch,
				Connector.IMPLIES);
		return wp;
	}
	...
	public Formula wpBranch(IJml2bConfiguration config,
		...
		Formula stackTop_grt_0 = new Predicate2Ar(
				new Stack(Expression.COUNTER), new NumberLiteral(0),
				PredicateSymbol.GRT);
		Formula grt_branch = (Formula) _normal_Postcondition.substitute(
				Expression.COUNTER, Expression.getCOUNTER_MINUS_1());
	...
	public VCGPath wp(IJml2bConfiguration config, VCGPath vcs, ExsuresTable _exc) {
		...
		Integer key = vcs.addHyp(getPosition(), stackTop_not_grt_0);
	...
	public VCGPath wpBranch(IJml2bConfiguration config, VCGPath vcs,
		...
		Formula stackTop_grt_0 = new Predicate2Ar(
				...
				PredicateSymbol.GRT);
		vcs.substitute(Expression.COUNTER, Expression.getCOUNTER_MINUS_1());
		Integer key = vcs.addHyp(getPosition(), stackTop_grt_0);
		vcs.addHypsToVCs(key);
		return vcs;
