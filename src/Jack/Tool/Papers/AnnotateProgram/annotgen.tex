...
\(v_1\) and \(v_2\) are the same
(however, this holds only if the property has not been breached, more on this
later).
...
%TODO: Try to avoid overfulls (the problem ofc is space)
\begin{figure}[t]
\[{\small
\begin{array}{rcl}
...
throw the \JMLExc.  Therefore, we cannot prove that the annotated
...
\emph{catch} block throws a \JMLExc, then the whole statement
