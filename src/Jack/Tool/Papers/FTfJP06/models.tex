...
\etab
Translated through Jack it would be linked with an \code{ObjectSet\_toSet} definition in Coq:
\btab
...
\etab
where \code{refelements} is a dereferencing relation and \code{arraylength} the relation to get the length of an array. This translation function is built around 2 functions:
\begin{itemize}
\item a first one (\code{ObjectSet\_toSet}) determining the value of the result if the \code{tab} parameter is \code{null} 
\item a recursive function (\code{toSet\_intern}) that add to the set each element from the array
\end{itemize}
...
(refelements tab i)). 
\etab
each element in the array \code{tab} is a member of the corresponding set defined by the translation
function.
\item
\btab
forall \=tab arraylength,\+\\
