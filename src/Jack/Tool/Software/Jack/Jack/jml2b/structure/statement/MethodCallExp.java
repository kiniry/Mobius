...
//					coqMethodParam = new BinaryForm(IFormToken.Ja_COMMA, new TerminalForm(IFormToken.Ja_LITERAL_this, "this"), coqMethodParam);
	...
	public Proofs wp(IJml2bConfiguration config, String result, Proofs normalBehaviour,
		...
		if (m instanceof Method && ((Method) m).isCoqDef()) {
			FormulaWithPureMethodDecl fw = exprToForm(config);
...
//		
		...
		while (e.hasMoreElements()) {
			...
			specRequires.old();
			FormulaWithPureMethodDecl specRequires1 = specRequires.instancie(new TerminalExp(xx, new Type(m
			...
			FormulaWithPureMethodDecl ensures = ensures1.instancie(new TerminalExp(xx, new Type(m.getDefiningClass())))
					.predToForm(config);
