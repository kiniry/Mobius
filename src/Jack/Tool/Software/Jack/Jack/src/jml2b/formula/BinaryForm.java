...
public class BinaryForm extends Formula {
	...
	/**
	 * Constructs a binary formula from a loaded jpo file.
	...
	public boolean is(Formula f) {
		if (getNodeType() == f.getNodeType()
			&& left.is(((BinaryForm) f).left)
			&& right.is(((BinaryForm) f).right))
			return true;
		else if (getNodeType() == IFormToken.Ja_AND_OP)
			return getLeft().is(f) || getRight().is(f);
		else return false;
	}
	...
	public void save(IJml2bConfiguration config, IOutputStream s, IJmlFile jf) throws IOException {
		...
		left.save(config, s, jf);
		right.save(config, s, jf);
	}
