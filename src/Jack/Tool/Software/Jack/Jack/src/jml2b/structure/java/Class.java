...
extends AClass
	...
	public transient Proofs staticInitLemmas = new Proofs();

	public transient Proofs wellDefInvLemmas = new Proofs();
...
//			Logger.get().println(this.getName() + ": I am native");
	 ...
	 * @param lemmas the lemmas associated to this class.
	 * @param constructors vector of <code>Method</code> elements 
	 *     corresponding to the constructors defined by this class.
	 * @param methods vector of <code>Method</code> elements 
	 *     corresponding to the methods defined by this class.
	 * 
	 * @see Method
	 */
	public Class(
		...
		super(null, null);
		this.name = name;
		this.staticInitLemmas = lemmas;
		this.wellDefInvLemmas = wellDefInvLemmas;
...
//	}
	...
	private FormulaWithSpecMethodDecl addGlobalMemberInvariant(
		...
		// ensures that the class is visible
		if (canSee(c)) {
			// the member invariant for class c
			Vector member = new Vector();

			Enumeration e = c.getMemberInvariants();
			while (e.hasMoreElements()) {
				Invariant i = (Invariant) e.nextElement();
				if (i.isVisibleFrom(this)) {
					JmlExpression tmp = i.getInvariant();
					member.add(tmp);
				}
			}
	...
	private SimpleLemma addGlobalMemberInvariantProof(
		IJml2bConfiguration config,
		SimpleLemma current,
		AClass c)
		throws Jml2bException, PogException {
		...
		if (canSee(c)) {
			...
			// to the current Expression.
			if (member.size() > 0) {
				current.addGoals(Pog.declare(config, c, member));
			}
		}
		return current;
	}
	...
	private FormulaWithSpecMethodDecl addGlobalMemberInvariant(
		...
		while (e.hasMoreElements()) {
			Package subpackage = (Package) e.nextElement();
			current = addGlobalMemberInvariant(config, current, subpackage);
		}
		...
		while (e.hasMoreElements()) {
			AClass c = (AClass) e.nextElement();
			current = addGlobalMemberInvariant(config, current, c);
		}
...
//		}
	...
	private SimpleLemma addGlobalMemberInvariantProof(
		...
		while (e.hasMoreElements()) {
			Package subpackage = (Package) e.nextElement();
			current =
				addGlobalMemberInvariantProof(config, current, subpackage);
		}
		...
		while (e.hasMoreElements()) {
			AClass c = (AClass) e.nextElement();
			current = addGlobalMemberInvariantProof(config, current, c);
		}
...
//}
	...
	public SimpleLemma getGlobalMemberInvariantProof(IJml2bConfiguration config)
		throws Jml2bException, PogException {
		return addGlobalMemberInvariantProof(
			config,
			new SimpleLemma(),
			((JavaLoader) config.getPackage()).getRoot());
	}
...
//	}
	...
	public int nbPo() {
		...
		Enumeration e = constructors.elements();
		while (e.hasMoreElements())
			res += ((Method) e.nextElement()).nbPo();
		e = methods.elements();
		while (e.hasMoreElements())
			res += ((Method) e.nextElement()).nbPo();
		return res
			+ (staticInitLemmas == null ? 0 : staticInitLemmas.nbPo())
			+ (wellDefInvLemmas == null ? 0 : wellDefInvLemmas.nbPo());
	}

	void purgeObvious() {
		Enumeration e = constructors.elements();
		while (e.hasMoreElements())
			 ((Method) e.nextElement()).purgeObvious();
		e = methods.elements();
		while (e.hasMoreElements())
			 ((Method) e.nextElement()).purgeObvious();
		if (staticInitLemmas != null)
			staticInitLemmas.proveObvious(true, true);
		if (wellDefInvLemmas != null)
			wellDefInvLemmas.proveObvious(true, true);
	}
	...
	public int nbPoProved(String prover) {
		...
		Enumeration e = constructors.elements();
		while (e.hasMoreElements())
			res += ((Method) e.nextElement()).nbPoProved(prover);
		e = methods.elements();
		while (e.hasMoreElements())
			res += ((Method) e.nextElement()).nbPoProved(prover);
		return res
			+ (staticInitLemmas == null
				? 0
				: staticInitLemmas.nbPoProved(prover))
			+ wellDefInvLemmas.nbPoProved(prover);
	}
	...
	public int nbPoProved() {
		...
		Enumeration e = constructors.elements();
		while (e.hasMoreElements())
			res += ((Method) e.nextElement()).nbPoProved();
		e = methods.elements();
		while (e.hasMoreElements())
			res += ((Method) e.nextElement()).nbPoProved();
		return res
			+ (staticInitLemmas == null ? 0 : staticInitLemmas.nbPoProved())
			+ wellDefInvLemmas.nbPoProved();
	}

	void mergeWith(jpov.structure.Class jf) {
		if (staticInitLemmas != null)
			staticInitLemmas.mergeWith(jf.getStaticInitLemmas());
		if (wellDefInvLemmas != null)
			wellDefInvLemmas.mergeWith(jf.getWellDefInvLemmas());
		Enumeration e = constructors.elements();
		while (e.hasMoreElements()) {
			Method m = (Method) e.nextElement();
			m.mergeWith(jf.getConstructors());
		}
		e = methods.elements();
		while (e.hasMoreElements()) {
			Method m = (Method) e.nextElement();
			m.mergeWith(jf.getMethods());
		}
	}

	public void mergeWith(jpov.structure.Class[] classes) {
		for (int i = 0; i < classes.length; i++) {
			jpov.structure.Class c = classes[i];
	...
	public String emit(IJml2bConfiguration config) {
		boolean b = false;
		String s = modifiers.emit();
		if (isInterface())
			s += "interface ";
		else
			s += "class ";
		s += getName();
		s += " extends " + superClass.getRefType().getName();
		if (implementedInterfaces.size() > 0) {
			s += " implements ";
			Enumeration e = implementedInterfaces.elements();
			while (e.hasMoreElements()) {
				Type element = (Type) e.nextElement();
				if (b)
					s += ",";
				else
					b = true;
				s += element.getRefType().getName();
			}
		}
		s += "{\n";
		Enumeration e1 = fields.elements();
		while (e1.hasMoreElements()) {
			Field element = (Field) e1.nextElement();
			s += element.emit();
		}
		s += "\n";
		e1 = allInvariants.elements();
		while (e1.hasMoreElements()) {
			Invariant element = (Invariant) e1.nextElement();
			s += element.emit() + "\n";
		}
		s += "\n";
		e1 = allConstraints.elements();
		while (e1.hasMoreElements()) {
			Invariant element = (Invariant) e1.nextElement();
			s += element.emit() + "\n";
		}
		s += "\n";
		e1 = constructors.elements();
		while (e1.hasMoreElements()) {
			Method element = (Method) e1.nextElement();
			s += element.emit(config) + "\n";
		}
		s += "\n";
		e1 = methods.elements();
		while (e1.hasMoreElements()) {
	...
	public void save(IJml2bConfiguration config, JpoOutputStream s, JmlFile jf) throws IOException {
		s.writeUTF(getName());
		s.writeInt(getFieldsV().size());
		Enumeration e1 = getFieldsV().elements();
		while (e1.hasMoreElements()) {
			Field field = (Field) e1.nextElement();
			String name = "";
			if (field.getModifiers() != null
				&& field.getModifiers().isStatic())
				name += field.getDefiningClass().getName() + ".";
			name += field.getName();

			s.writeUTF(name);
		}
		getStaticInitLemmas().save(config, s, jf);
		getWellDefInvLemmas().save(config, s, jf);
		int cpt = 0;
		e1 = getConstructors();
		while (e1.hasMoreElements()) {
			e1.nextElement();
