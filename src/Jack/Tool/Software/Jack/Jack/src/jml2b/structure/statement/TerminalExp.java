...
public class TerminalExp extends Expression {
	...
	/**
	 * Returns the normal behaviour proof where the resulting variable has been 
	...
	public Proofs wp(
		...
		switch (getNodeType()) {
			case MyToken.BTRUE :
				f = new TerminalForm(Ja_LITERAL_true);
				break;
			default :
				f = exprToForm(config).getFormula();
		}
		return ((Proofs) normalBehaviour.clone()).addBox(
			new ColoredInfo(this)).sub(
			new SubTmpVar(result, f));
	}
