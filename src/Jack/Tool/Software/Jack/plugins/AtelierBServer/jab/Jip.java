...
public class Jip {
	...
	public Jip(String project, String name, boolean debug) {
		this.debug = debug;
		try {

			p = Runtime.getRuntime().exec(Jab.exec);
			...
			String[] possibleres =
				{
					"\nNo current PO\n",
					"\nSomeone is modifying component " + name };
			String res =
				sendCommand(
					"op " + project + "\n" + "b " + name + "\n",
					possibleres);
	...
	String sendCommand(String command, String[] response) {
		...
		try {
			...
			int i;
			lab : do {
				while ((i = is.available()) == 0 && (er.available()) == 0);
				if (i != 0) {
					b = new byte[i];
					is.read(b, 0, i);
					res += new String(b);
	...
	public boolean isProved(String name, int po) {
		...
		String sub2 =
			sub1.substring(
				sub1.indexOf("PO" + po) + ("PO" + po).length() + 1,
