...
public class BCIFEQ extends BCConditionalBranch {
	...
	public Formula wp(IJml2bConfiguration config,
		...
		Formula stackTop_not_eq_0 = Formula.getFormula(new Predicate2Ar(
				new Stack(Expression.COUNTER), new NumberLiteral(0),
				PredicateSymbol.EQ), Connector.NOT);
		Formula not_eq_branch = (Formula) _normal_Postcondition.substitute(
				Expression.COUNTER, Expression.getCOUNTER_MINUS_1());
		wp = Formula.getFormula(stackTop_not_eq_0, not_eq_branch,
				Connector.IMPLIES);
		return wp;
	}
	...
	public Formula wpBranch(IJml2bConfiguration config,
		...
		Formula stackTop_eq_0 = new Predicate2Ar(new Stack(Expression.COUNTER),
				new NumberLiteral(0), PredicateSymbol.EQ);

		Formula eq_branch = (Formula) _normal_Postcondition.substitute(
				Expression.COUNTER, Expression.getCOUNTER_MINUS_1());
		wp = Formula.getFormula(stackTop_eq_0, eq_branch, Connector.IMPLIES);
	...
	public VCGPath wp(IJml2bConfiguration config, VCGPath vcs, ExsuresTable _exc) {
		...
		Integer key = vcs.addHyp(getPosition(), stackTop_not_eq_0);
	...
	public VCGPath wpBranch(IJml2bConfiguration config, VCGPath vcs,
		...
		vcs.substitute(Expression.COUNTER, Expression.getCOUNTER_MINUS_1());
		Integer key = vcs.addHyp(getPosition(), stackTop_eq_0);
		vcs.addHypsToVCs(key);
		return vcs;
	}
