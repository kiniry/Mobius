...
import org.gjt.jclasslib.browser.detail.iflowattributes.SLFieldAttributePane;
import org.gjt.jclasslib.browser.detail.iflowattributes.SLMethodAttributePane;
...
import org.gjt.jclasslib.structures.iflowattributes.ExceptionSafeAttributePane;
...
/**
    ...
    @version $Revision: 1.5 $ $Date: 2006/05/30 08:54:40 $
*/
public class AttributeDetailPane extends AbstractDetailPane {
	...
	private static final String SCREEN_CONSTRAINT = "Constraint";
        // added for Iflow
    	private static final String SCREEN_SL_FIELD = "Field Security Level";
    	private static final String SCREEN_SL_METHOD = "Method Security Specification";
    	private static final String SCREEN_EXCEPTION_SAFE = "Exception Safe";
	...
	public void show(TreePath treePath) {
		...
		} else if (attribute instanceof SecLevelFieldAttribute) {
			paneName = SCREEN_SL_FIELD;			
		} else if (attribute instanceof SecLevelMethodAttribute) {
			paneName = SCREEN_SL_METHOD;			
		} else if (attribute instanceof ExceptionSafeAttribute){
		    paneName = SCREEN_EXCEPTION_SAFE;
		}
	...
	private void buildSpecificInfoPane() {
		...
		// added for Iflow
		addScreen(
			  new SLFieldAttributePane(services),
			  SCREEN_SL_FIELD);
		addScreen(
			  new SLMethodAttributePane(services),
			  SCREEN_SL_METHOD);
		addScreen(
			  new ExceptionSafeAttributePane(services),
			  SCREEN_EXCEPTION_SAFE);
	}
