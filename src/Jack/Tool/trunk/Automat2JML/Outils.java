...
public class Outils {
	...
	private final String[] synchroTypeString = { "Normal Terminasion", "Except Terminasion", "Call", };
	...
	public LinkedList extractInstances(Node node, LinkedList propertiesModel){
		...
		while (voidMatcher.find()){	//As long as there are instance to extract
			...
			//If it is an instance
				...
				if (instanceName.compareToIgnoreCase(instanceMatcher.group(1)) != 0){
					methodDefinition = "";} //method Declaration doesn't correspond anymore to the funtion
				instanceName = instanceMatcher.group(1);
				objectInstantiated = instanceMatcher.group(2);
				instanceParam = instanceMatcher.group(3);
				
				//if it is a method that is instanciated (identified by the "MethodBehavior" key word) 
				if (objectInstantiated.compareToIgnoreCase("methodbehavior")==0){ 
					//System.out.println("Found method "+methodName+" belonging to "+className);
					...
					//System.err.println("editParamLinkList called for "+method);
		...
		while (it1.hasNext()){
			...
			while (it2.hasNext()){
				...
				if (paramLink == null) {System.err.println("Impossible to link transition "+transition+
						"(of prop: "+prop.propertyName+") with any method - might be another automaton but feature is not supported yet");return false;}		
