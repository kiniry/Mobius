...
BML annotations generated with described translation mechanism must be added to the output Java class file. Some BML annotations are connected with class or method e.g. invariants. This case is simple. More problematic are asserts and loop invariants. Each assert clause needs to be assigned to proper bytecode instruction. For positioning assert clauses \emph{line number table} is used. This structure, present in Java class files, associates lines in source file with bytecode instructions. BML assert clauses are assigned to the first bytecode instruction which realises the particular line.

Loop invariant must be associated with bytecode instruction too. Loop invariant holds at the beginning of the loop
body at each loop iteration. Taking into consideration that calculating loop arguments does not have to be site effect free,
loop invariant should not be placed before calculating the arguments. Therefore the proper bytecode instruction to attach the loop invariant to is the first instruction after the loop arguments are calculated. To find such instruction there is a need to link instructions from the source code with corresponding bytecode instructions from the provided class file. 
...
then augment them with specifications and subsequently move the specifications to the
...
can then be proved by different proving technologies \cite{mobius31,bml2bpl}.
