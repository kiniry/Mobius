...
example of a forerunner of thich technique. Although the certificate
...
of the multiple pairs is such that for each pair such that the
requires formula holds at the entry to the method, the corresponding
ensures formula must hold at the exit (so effectively the conjunction
of all the ensures formulas holds). However, we often want to specify
...
in all the cases. In our JML code, this clause is implicit and
equivalent to \texttt{true} whereas in BML this is made explicit as
the formula after the \texttt{requires} keyword. This also is why the
actual JML precondition is translated to the internal
\texttt{precondition} statement (it is one of potentially many
preconditions).

Loops specifications are located after line 32 in the presented
...
detecting loops refer to Section~\ref{sec:loops}. The modifies
clause describes set of variables modified by the loop. Currently,
because of \openjml limitations, it is not supported by our
compiler (the default value \texttt{everything} will be inserted).

\section{\jmltobmltext{} Compiler Design}
\label{sec:compiler}

In our work we have designed and implemented a tool called \jmltobml
that compiles JML specifications into BML. It takes as input a Java
source file with JML annotations together with the corresponding class
file and outputs the class file with inserted proper BML
annotations. Our compiler uses an enhanced Abstract Syntax Tree (AST)
for the Java source code, taken from the \openjml\footnote{Available
from \url{http://sourceforge.net/projects/jmlspecs}} compiler (a JML
checker based upon the OpenJDK Java tool set). For different types of
JML clauses, there are separate translation rules defined. At each
node of the AST, all translation rules are applied. If some rule
succeeds to translate this node, the result is stored in to the class
file, using the \bmllib library \cite{bmllib}. This approach makes the
compiler easily extensible. One can simply write new translation rule
to support additional features of the JML language.
...
\openjml and \bmllib are presented in Figure~\ref{fig:packages}.
...
\subsection{Example of Translation Rule}

\lstset{language=java,
        basicstyle=\scriptsize,commentstyle=\scriptsize,moredelim=*[s][\scriptsize]{/*@}{*/},
        numbers=left,numberstyle=\tiny,stepnumber=1,numbersep=5pt,escapeinside={(*@}{@*)}}
\begin{figure}[htbp]
~~~~~~%
      ...
      if (classInvariant == null) {
        ...
        classInvariant =
            new ClassInvariant(clazz,
                               newFormula); (*@\label{rule_update_invariant}@*)
      }
...
\end{figure}
Figure~\ref{fig:translation_rule_example} contains example implementation of translation rule - for class invariants. The rule extends \texttt{TranslationRule} class. It has one attribute \texttt{context}, which holds the context in which rule will be executed. Class invariants in OpenJML abstract syntax tree are represented by \texttt{JmlTypeClauseExpr} class nodes. As we use visitor pattern, we have to override method responsible for these nodes - \texttt{visitJmlTypeClauseExpr}.

Implementation of translation method is simple. First we check if this is the node type we are interested in - line \ref{rule_token}. We get an object representing the class which we want to add invariant to (line \ref{rule_class}). Next, in line \ref{rule_formula}, we translate the content of invariant - rule for translating expression is used here. In line \ref{rule_get_invariant} existing class invariant is taken from the class. If there was no invariant - new one is created (line \ref{rule_new_invariant}), otherwise we connect old invariant formula with new one using \texttt{and} (line \ref{rule_new_formula}) to create new class invariant - line \ref{rule_update_invariant}. At the end, in line \ref{rule_set_invariant} we set new invariant to the class.
...
In some cases it is crucial to use the original, provided by user class file, the Jml2Bml compiler is therefore not allowed to compile the source code on its own. There is a need to link instructions from the source code with corresponding bytecode instructions from the provided class file. The most difficult and important part is to detect loops in the bytecode.
...
(comparison). A loop can be translated in one of the ways presented on
...
usually compiled in the way presented in Figure~\ref{fig:dowhile}
...
\item{find the first instruction after \textit{a} that has an incoming backward edge starting in an vertex after \textit{c}. This is our \textit{b}. Find the
