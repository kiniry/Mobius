...
\begin{abstract}
  ...
  architecture which allows to express detailed properties of Java
...
verify the lack of the null-pointer exceptions then the checks for null
...
environment) before the code is executed (see
...
In Sect.~\ref{sec:specification}, we present the specification
...
compiler. The problem of placement of the loop specification in the bytecode is discussed in Sect.~\ref{sec:loops}. The related work is presented in
...
be precisely documented using JML annotations. They describe the
invariant properties that are maintained by objects, the input method
...
precisely specify the desired behaviour of the components which is
crucial in case they must be combined with as little implementors'
...
In order to achieve easy readability for programmers, JML stays as
close as possible to the Java syntax and semantics. The tool support
for JML is rich (see~\cite{overviewOfJML} for an overview). In
...
Java bytecode. It was proposed by Burdy et al. in~\cite{bmlBurdy}. The
...
The class files with BML annotations are regular Java class files,
...
representation of BML specifications. However, the binary class
...
(BML requires to add explicit \texttt{this} prefix to fields) and the
\texttt{$\backslash$old} operator becomes a part of \texttt{this}
reference).
...
The rules in the implementation of \jmltobml are based on abstract compilation
...
In the former case, the unlimited acess to the source code may suggest
...
exploited to check that the code indeed obeys the required policiy is
...
as it allows to associate formulae not only to types or code points
...
However, BML specification language and the attribute
