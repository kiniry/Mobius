...
public class Main {
	 ...
	 */
	public void compile(String sourceFile, String classFile) throws ClassNotFoundException, ReadAttributeException {
		Context context = Factory.getContext(new BCClass(ProjectDirectory.PROJECT_DIR+"\\bin", "experiments.Test"));
		JavaCompiler compiler = JavaCompiler.instance(context);
		JCCompilationUnit tree = compiler.parse(getJavaFileObject(context, sourceFile));
		ParentFinder parentFinder = new ParentFinder();
		tree.accept(parentFinder, null);
		Map<Tree, Tree> parents = parentFinder.getParents();
		Jml2BmlTranslator translator = TranslationManager.getTranslator(context, parents);
	...
	private JavaFileObject getJavaFileObject(Context context, String filename){
		...
		return fm.getJavaFileObjectsFromStrings(List.of(filename)).iterator().next();
	}
}
