package bonIDE.diagram.parsers;
...
public abstract class AbstractParser implements IParser {
	 ...
	 * @generated
	 */
	protected ICommand getParseCommand(
			IAdaptable adapter, Object[] values, int flags) {
		if (values == null || validateNewValues(values).getCode() !=
				IParserEditStatus.EDITABLE) {
	 ...
	 * @generated
	 */
	protected ICommand getModificationCommand(
			EObject element, EAttribute feature, Object value) {
		value = getValidNewValue(feature, value);
		if (value instanceof InvalidValue) {
			return UnexecutableCommand.INSTANCE;
		}
		SetRequest request =
				new SetRequest(element, feature, value);
		...
		if (type instanceof EDataType) {
			...
			if (Boolean.TYPE.equals(iClass)) {
				if (value instanceof Boolean) {
					...
					value = new InvalidValue(NLS.bind(bonIDE.diagram.part.Messages.AbstractParser_UnexpectedValueType,
							iClass.getName()));
				}
					...
					if (s.length() == 0) {
						...
						try {
							...
							value = new InvalidValue(NLS
									.bind(bonIDE.diagram.part.Messages.AbstractParser_WrongStringConversion, iClass
											.getName()));
						}
					}
				} else {
				...
				if (value instanceof String) {
					EEnumLiteral literal =
							((EEnum) type).getEEnumLiteralByLiteral((String) value);
					if (literal == null) {
						value = new InvalidValue(NLS.bind(bonIDE.diagram.part.Messages.AbstractParser_UnknownLiteral,
								value));
					} else {
