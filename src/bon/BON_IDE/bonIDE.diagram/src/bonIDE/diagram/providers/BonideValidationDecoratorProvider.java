 ...
 * @generated
 */
public class BonideValidationDecoratorProvider
	 ...
	 * @generated
	 */
	private static final String MARKER_TYPE = bonIDE.diagram.part.BonideDiagramEditorPlugin.ID + ".diagnostic"; //$NON-NLS-1$
	...
	public void createDecorators(IDecoratorTarget decoratorTarget) {
		...
		if (editPart instanceof GraphicalEditPart ||
			...
			if (((DiagramEditDomain) ed).getEditorPart() instanceof bonIDE.diagram.part.BonideDiagramEditor) {
	...
	public boolean provides(IOperation operation) {
		...
		View view = (View) decoratorTarget.getAdapter(
				View.class);
		return view != null && bonIDE.diagram.edit.parts.ModelEditPart.MODEL_ID.equals(bonIDE.diagram.part.BonideVisualIDRegistry.getModelID(view));
	}
		...
		PlatformUI.getWorkbench().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				try {
					...
					bonIDE.diagram.part.BonideDiagramEditorPlugin.getInstance().logError("Decorator refresh failure", e); //$NON-NLS-1$
				}
			}
		});
	}
	...
	public static class StatusDecorator extends AbstractDecorator {
		...
		public StatusDecorator(IDecoratorTarget decoratorTarget) {
			...
			try {
				...
				bonIDE.diagram.part.BonideDiagramEditorPlugin.getInstance().logError("ViewID access failure", e); //$NON-NLS-1$			
			}
		}
		...
		public void refresh() {
			removeDecoration();
			View view = (View) getDecoratorTarget().getAdapter(
					View.class);
			...
			try {
				...
				bonIDE.diagram.part.BonideDiagramEditorPlugin.getInstance().logError("Validation markers refresh failure", e); //$NON-NLS-1$
			}
	...
	static class MarkerObserver implements IFileObserver {
		...
		private String getType(IMarker marker) {
			try {
				...
				bonIDE.diagram.part.BonideDiagramEditorPlugin.getInstance().logError("Validation marker refresh failure", e); //$NON-NLS-1$
				return ""; //$NON-NLS-1$
			}
		}
	}
