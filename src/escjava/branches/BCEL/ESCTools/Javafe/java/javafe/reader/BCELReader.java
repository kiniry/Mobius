...
class BCELReader extends Reader {
	...
	protected CompilationUnit convertBCELtoAST() throws ClassNotFoundException,
		...
		// Loading the parsed class so that reflection can be used
		String className = javaClass.getClassName();
		classToParse = Class.forName(className);
		...
		Name pkgName = Name
				.make(javaClass.getPackageName(), classLocationIndex);
		...
		CompilationUnit result = CompilationUnit.make(pkgName, null,
				emptyImportDeclVec, types, classLocationIndex,
		...
		for (int loopVar = 0; loopVar < numberOfMethodsInClass; loopVar++) {
			...
			for (int attributeLoopVar = 0; attributeLoopVar < attributes.length; attributeLoopVar++) {
				...
				if (attributeName.equals("Exceptions")) {
					routineDecl[attributeLoopVar].raises = TypeNameVec
							.make(parseTypeNames());
	 ...
	 * @return
	 */
	protected String getAttributeName(Attribute[] attributes,
			int attributeLoopVar) {
		...
		Constant attributeNameConstant = attributeConstantPool
				.getConstant(attributeNameIndex);
	 ...
	 * @throws ClassFormatError
	 */
	protected void set_class_attribute() throws IOException, ClassFormatError {
		...
		for (int loopVar = 0; loopVar < classAttributes.length; loopVar++) {
			...
			if (attributeName.equals("Synthetic")) {
				...
				for (int j = 0; j < getNumInnerClasses(); j++) {
					int innerClassIndex = getInnerClassIndex(j);
					// @ assume inner < constants.length; // property of class
					// files
					int outerClassIndex = getOuterClassIndex(j);
					int classNameIndex = javaClass.getClassNameIndex();
					// @ assume name < constants.length; // property of class
					// files
					int flags;
					// System.out.println("PPP" + Modifiers.toString(flags));
					if (outerClassIndex == this_class_index
							&& classNameIndex != 0) {
						// We've found a member that needs to be parsed...
						if (!(rawConstants[classNameIndex] instanceof String)) {
							throw new ClassFormatError("bad constant reference");
						}
						// @ assume rawConstants[inner] instanceof String; //
						// property of class files
						String nm = (String) rawConstants[innerClassIndex];
						int i = nm.lastIndexOf("/");
						String icfn = (i < 0 ? nm : nm.substring(i + 1))
								+ ".class";
						GenericFile icf = inputFile.getSibling(icfn);
						if (icf == null) {
							throw new IOException(icfn
						...
						// Only add classes that are not synthetic and are not
						// anonymous inner classes,
						// which are identified by names that start with a
						// number.
						if (!parser.syntheticClass
								&& !Character.isDigit(parser.typeDecl.id
										.toString().charAt(0))) {
	...
	/**
	 * Get the number of inner classes for this class
	 * 
	 * @return
	 */
	protected int getNumInnerClasses() {
	 ...
	 * @throws ClassFormatError
	 */
	protected void readField(int fieldIndex, Field field)
			throws ClassFormatError {

		String fieldName = field.getName();
		javafe.ast.Type astType = readType(field);
		int fieldModifiers = field.getModifiers();

		fieldDecl[fieldIndex] = // @ nowarn IndexTooBig;
		FieldDecl.make(fieldModifiers, null, Identifier.intern(fieldName),
				astType, classLocation, null, classLocation);
	}
	...
	protected javafe.ast.Type readType(Field field) {
		...
		// Non primitive types require extra work
		switch (typeTag) {

		case TagConstants.ARRAYTYPE:
			astType = readArrayType(field);

		case TagConstants.NULLTYPE:
			astType = readObjectType(field);
	 ...
	 * @return
	 */
	protected javafe.ast.Type readArrayType(Field field) {
		// TODO Auto-generated method stub
		return null;
	}
	...
	public CompilationUnit read(GenericFile target, boolean avoidSpec) {
		...
		try {
			...
			CompilationUnit compilationUnit = convertBCELtoAST();
