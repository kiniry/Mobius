...
objects to parse pragmas out of pragma-containing comments.  They are
also called to see if a comment contains pragmas in the first place.
See <code>Lex</code> for more details.
...
<code>TagConstants.TYPEDECLELEMPRAGMA</code>; the <code>auxVal</code>
field must be filled have a type according to the table in
<code>Token</code>.
...
public interface PragmaParser {
  ...
  */
  ...
  /** Parse the next pragma.  If none are left, returns
    ...
    updates fields of <code>destination</code> to describe the pragma.
    When <code>false</code> is returned, the pragma parser is expected
    to close the underlying <code>CorrelatedReader</code> and in other
    ways clean up resources.
