...
\date[Spring 2008]{March 27, 2008 / \href{http://www.jmlspecs.org/}{JML} Tutorial / \href{http://www.jmlspecs.org}{jmlspecs.org}}
...
\item
ESC/Java2 \\
\href{http://kind.ucd.ie/products/opensource/ESCJava2/}{http://kind.ucd.ie/products/opensource/ESCJava2/}

\item
The Mobius Program Verification Environment (PVE) \\
\href{http://kind.ucd.ie/products/opensource/Mobius/}{http://kind.ucd.ie/products/opensource/Mobius/}
\end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Strengths of Extended Static Checking}
\begin{itemize}
\item
...
(Actually JML has a way to apply constraints only to certain named methods.)
}
\end{frame}
...
\note{Note the problem with the last assert, need frame for \texttt{moveUp} to
  reason that the change to \texttt{x} is not overwritten by \texttt{moveUp}.

  So need to have assignable clause for \texttt{moveUp}.

  Actually, you also need an assignable clause for \texttt{moveRight}
  in this example also.  Do you see why?
}
\end{frame}
