    ...
    implements java.io.Serializable, Comparable, CharSequence
{
    ...
    private char value[]; //@ in stringSeq;
    //@ private represents stringSeq <- value;

    /** The offset is the first index of the storage that is used. */
    private int offset; //@ in stringSeq;

    /** The count is the number of characters in the String. */
    private int count; //@ in stringSeq;
    //@ private represents size <- count;

    /** Cache the hash code for the string */
    private int hash = 0; //@ in stringSeq;
    ...
    //@   requires !(length < 0 || offset < 0 || (bytes != null && (offset > bytes.length - length)));
    //@ also
    //@ private exceptional_behavior
    //@   requires (length < 0 || offset < 0 || (bytes != null && (offset > bytes.length - length)));
    //@   signals (StringIndexOutOfBoundsException);
    private static /*@ pure @*/ void checkBounds(byte[] bytes, int offset, int length) {
	if (length < 0)
