/**
 ** DecreasesInfoVec: A Vector of DecreasesInfo's. <p>
 **
 ...
 ** $Id: DecreasesInfoVec.j 1928 2006-05-19 12:43:10Z chalin $
 **
 ** DO NOT EDIT!!<p>
 **/

/*
 * File: DecreasesInfoVec.java <p>
 *
 ...
 *
 * DecreasesInfo should be globally replaced with the simple name of the
 *		element type<p>
 ...
 *
 * The resulting type will have name DecreasesInfoVec.<p>
 * 
 ...
 *
 *
 * By Default, DecreasesInfoVec's may not contain nulls.  If the ability to
...
public class  DecreasesInfoVec {
    ...
    private /*@ spec_public non_null*/ DecreasesInfo[] elements;
    //@ invariant (\forall int i; (0<=i && i<count) ==> elements[i]!=null);
    //@ invariant \typeof(elements) == \type(DecreasesInfo[]);
	...
	this.elements = new DecreasesInfo[count];
	...
	this.elements = new DecreasesInfo[(cnt == 0 ? 2 : cnt)];
	//@ set elements.owner = this;
	...
	return new DecreasesInfoVec(count);
    }

    //@ requires vec!=null;
    //@ requires vec.elementType <: \type(DecreasesInfo);
    //@ requires !vec.containsNull;
    //@ ensures \result!=null;
    //@ ensures \fresh(\result);
    public static DecreasesInfoVec make(Vector vec) {
	int sz = vec.size();
	DecreasesInfoVec result = new DecreasesInfoVec(sz);
	result.count = sz;
	...
	return new DecreasesInfoVec(els);
    }

    //
    //@ requires s.vectorCount>1;
    //@ requires s.elementType <: \type(DecreasesInfo);
    //@ ensures \result!=null;
    ...
    public static DecreasesInfoVec popFromStackVector(/*@non_null*/ StackVector s) {
	// Creates a new DecreasesInfoVec from top stuff in StackVector
	int sz = s.size();
	DecreasesInfoVec r = new DecreasesInfoVec(sz);
  ...
  public final DecreasesInfo[] toArray() {
      int ct = count;
      DecreasesInfo[] b = new DecreasesInfo[ ct ];
      for(int i = 0; i < ct; i++) {
	  b[i]=elements[i];
      }
      return b;
  ...
  public final DecreasesInfoVec copy() {
    DecreasesInfoVec result = new DecreasesInfoVec(count);
    result.count = count;
  ...
  public final void addElement(DecreasesInfo x) {
    if( count == elements.length ) {
      DecreasesInfo[] newElements = new DecreasesInfo[ 2*(elements.length==0 ?
					      2 : elements.length) ];
  ...
  public final boolean removeElement(DecreasesInfo x) {
    ...
    public final DecreasesInfo pop() {
        count--;
        DecreasesInfo result = elements[count];
	elements[count] = null;
	return result;
    }
