...
/**
 * * This class contains a number of routines used in the * interpretation of
 * Java 1.1 as Java 1.0. *
 */
...
public class Inner {
  ...
  /**
   * * Decorates TypeSig nodes to point to their enclosing instance * field
   * (this$0).
   * <p>
   */
  //@ invariant enclosingInstanceDecoration.decorationType == \type(FieldDecl);
  //@ spec_public
  private static final ASTDecoration enclosingInstanceDecoration = new ASTDecoration(
      "enclosingInstanceDecoration");

  /**
   * * Return the FieldDecl for a given non-top-level TypeSig's * enclosing
   * instance field (this$0).
   * <p>* * The resulting field is public, final, and non_null.
   * <p>
   */
  //@ ensures \result != null;
  public static FieldDecl getEnclosingInstanceField(TypeSig T) {
    ...
    pmodifiers
        .addElement(SimpleModifierPragma.make(TagConstants.NON_NULL, loc));

    result = FieldDecl.make(Modifiers.ACC_PUBLIC | Modifiers.ACC_FINAL,
        pmodifiers, Identifier.intern(name), T.enclosingType, // the Type of the
                                                              // decl...
        loc, null, Location.NULL); // has no initializer
   ...
   */
  //@ spec_public
  private static final ASTDecoration enclosingInstanceArgument = new ASTDecoration(
   ...
   * <p>* * The resulting argument is non_null.
   * <p>* * WARNING: Translate.call depends on the exact name of * this$0arg.
   * If you change this$0arg's name, you must change * Translate.call
   * accordingly.
   * <p>
   */
  //@ ensures \result != null;
  public static FormalParaDecl getEnclosingInstanceArg(ConstructorDecl cd) {
    // Memorize our result:
    FormalParaDecl result = (FormalParaDecl)enclosingInstanceArgument.get(cd);
    if (result != null) return result;
    ...
    result = FormalParaDecl.make(Modifiers.NONE, pmodifiers, Identifier
        .intern(name), T.enclosingType, //the Type of the argument...
        cd.locId);
  ...
  /**
   * * If non-null, the local variable or formal to use instead of * this.this$0
   * when unfolding <class>.this's.
   */
  static GenericVarDecl firstThis0 = null;

  /**
   * * Converts a 1.1 ThisExpr of the form <class>.this into an Java * 1.0
   * expression of the form this.this$0.this$0.this$0...
   * <p>* * The type of this is taken from GC.thisvar.decl.type.
   * <p>* * The this$0 fields are the appropriate fields from the *
   * getEnclosingInstanceField(-) routine above.
   * <p>* * The resulting Expr has already been "prepped", but *not* type *
   * checked.
   * <p>* * * Exception: If firstThis0 is non-null, then it is used instead *
   * of this.this$0 at the start.
   * <p>
   */
  ...
  static Expr unfoldThis(/* @non_null */ThisExpr t) {
    ...
    for (int level = 0; thisType != target; level++) {
      /*
       * add a .this$0 to result, using firstThis0 the first time if non-null:
       */
      if (level == 0 && firstThis0 != null)
        ...
        result = FieldAccess.make(ExprObjectDesignator.make(t.loc, result),
            thisDollar.id, t.loc);
