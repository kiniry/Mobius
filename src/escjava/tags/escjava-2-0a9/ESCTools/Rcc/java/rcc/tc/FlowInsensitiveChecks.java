...
public class FlowInsensitiveChecks extends javafe.tc.FlowInsensitiveChecks {
     ...
     **
     ** (Used by GhostEnv and requires/guarded annotations)<p>
     **/
    public static boolean inAnnotation = false;
    
    public static final ASTDecoration guardDecoration = new ASTDecoration("guard");
    public static final ASTDecoration requiresDecoration = new ASTDecoration("requires");
    public static final ASTDecoration elemGuardDecoration = new ASTDecoration("element guard");
    public static final ASTDecoration threadLocalDecoration = new ASTDecoration("local status");
	...
	EnvForTypeSig env = new GhostEnv(s.getEnv(staticContext), s, staticContext);
	return env;
    }
  ...
  protected void checkTypeDeclElem(TypeDeclElem e) {
	...
	if (fd.init != null) {
	    ...
	    if (rcc.Main.ihl) {
		if (staticContext) {
		    locks.push(ClassLiteral.
			       make(TypeSig.getSig(fd.getParent()),
				    fd.getStartLoc()));
		} else {
		    locks.push(ThisExpr.make(sig, fd.getStartLoc()));
		}
	    }
	    if (rcc.Main.ihnl && staticContext) {
		locks.push(LiteralExpr.make( TagConstants.NULLLIT, null, 
					      fd.getStartLoc()));
	    }
	...
	break;
      ...
      case TagConstants.CONSTRUCTORDECL:
		...
		for( int j=0; j<rd.raises.size(); j++ ) {
		    ...
		    allowedExceptions.addElement( TypeSig.getSig( n ) ); //@ nowarn Pre
		}
	...
	if (rcc.Main.ihnl && staticContext) {
	    locks.push(LiteralExpr.
		       make(TagConstants.NULLLIT, null,
			    si.getStartLoc()));
	}
	returnType = null;
	canAssignReadOnly = true;
    ...
    public void checkModifierPragmaVec(ModifierPragmaVec mod,
				       ASTNode a,
				       Env env) {
	...
	if (expressions==null) {
	    ...
	    //    System.out.println("  " + env.isStaticContext()+ env + env.getEnclosingClass());
    ...
    public void  checkTypeModifierPragmaVec(TypeModifierPragmaVec mod,
					    ASTNode a,
					    Env env) {
    ...
    protected void checkTypeModifierPragma(TypeModifierPragma p, ASTNode ctxt, Env env) {
	...
	switch(tag) 
	    ...
	    case TagConstants.ARRAYGUARDMODIFIERPRAGMA:
		{
			...
			ErrorMsg.print(sig, "Modifiers",
				       ctxt.getStartLoc(),"only array declarations may contain this type of annotation.",
				       Location.NULL);
		    }
		    ExprVec g = (ExprVec)elemGuardDecoration.get(ctxt);
		    for (int i = 0;
			 i<rp.expressions.size(); i++) {
	...
	case TagConstants.CONSTRUCTORINVOCATION:
		...
		if (!inAnnotation ) {
		    ...
		    checkLocksHeld(ms, expressions,ci.getStartLoc(), "", ci.decl.pmodifiers);
		}
		return env;
	    }
	...
	inAnnotation = true;
	FormalParaDeclVec fpv = 
	    (FormalParaDeclVec)PrepTypeDeclaration.typeParametersDecoration.get(sig);

	ExprVec args = ExprVec.make();
	if (fpv != null) {
	    for (int i = 0; i<fpv.size(); i++) {
		FormalParaDecl parameter = fpv.elementAt(i);
		ExprObjectDesignator eod = ExprObjectDesignator.
		...
		FieldAccess fa = FieldAccess.
		    make(
			 ...
			 parameter.getStartLoc());
		args.addElement(fa);
		sig.getEnv(true).resolveType(parameter.type);
		setType(fa, parameter.type);
		fa.decl = (FieldDecl)PrepTypeDeclaration.
		    parameterDeclDecoration.get(parameter);
		setType(eod.expr, sig);   // good enough for now.  change below to correct sig
	    }
	}
	TypeSig s = 
	    ((rcc.tc.PrepTypeDeclaration)PrepTypeDeclaration.inst).
                 findTypeSignature(sig.getEnv(true), 
				   (rcc.tc.TypeSig)sig, 
				   args, 
	...
	for (int i = 0; i<args.size(); i++) {
	    ...
	    s.getEnv(true).resolveType(getType(fa));
	}
    ...
    protected void  checkLocksHeld(MultipleSubstitution ms, 
				   ExprVec expressions, 
				   int line, 
				   String msg, 
	...
	if (type instanceof ArrayType) {
		...
		modifyEscapingType(env, s, ((ArrayType)type).elemType);
	    return type;
	}
	...
	case TagConstants.THISEXPR: {
		...
		checkTypeModifiers(env, referredType);   // SNF Fri Jul 30 13:36:44 1999
	...
	case TagConstants.FIELDACCESS:
		...
		if (!inAnnotation) {
			...
			fa.decl.parent != null) {
			s.addElement(new 
				     Substitution(ThisExpr.make(TypeSig.getSig(fa.decl.parent),
								fa.od.getStartLoc()), 
		    ...
		    MultipleSubstitution ms = new MultipleSubstitution(s);
		    checkLocksHeld(ms, expressions,fa.getStartLoc(), "", fa.decl.pmodifiers);
		    return e;
		}
	...
	case TagConstants.METHODINVOCATION:
	    {
		e =  super.checkExpr(env, e);
		if (e==null) return e;  // SNF
		if (!inAnnotation ) {
		    MethodInvocation m = (MethodInvocation)e;
		    if (m.decl == null) return e;		    
		    ExprVec expressions = getRequiresVec(m.decl);
		    boolean staticContext = Modifiers.isStatic(m.decl.modifiers);
		    SubstitutionVec s = SubstitutionVec.make();
		    if (!staticContext && m.od.getTag()==TagConstants.EXPROBJECTDESIGNATOR) {
			s.addElement(new 
				     Substitution(ThisExpr.make(TypeSig.getSig(m.decl.parent), 
								m.od.getStartLoc()), 
			...
			FieldAccess fa = FieldAccess.
			    make(
				 ...
				 make(parameter.getStartLoc(),
				      ThisExpr.make(TypeSig.getSig(m.decl.parent),
						    parameter.getStartLoc())),
				 parameter.id,
				 parameter.getStartLoc());
			s.addElement(new Substitution(fa, expr));

			s.addElement(new Substitution(VariableAccess.
						      make(
							   parameter.id,
							   parameter.getStartLoc(),parameter),
						      expr));
		    }
		    modifyEscapingExpr(env, s,e);
		    		    
		    MultipleSubstitution ms = new MultipleSubstitution(s);
		    checkLocksHeld(ms, expressions,m.getStartLoc(), "", m.decl.pmodifiers);
		}
		return e;
	    }
		...
		if (!inAnnotation ) {
			...
			FieldAccess fa = FieldAccess.
			    make(
				 ExprObjectDesignator.
				 make(parameter.getStartLoc(),
				      ThisExpr.make(TypeSig.getSig(ne.decl.parent),
						    parameter.getStartLoc())),
				 parameter.id,
				 parameter.getStartLoc());
			s.addElement(new Substitution(fa, expr));
		    ...
		    modifyEscapingExpr(env, s,e);
		    MultipleSubstitution ms = new MultipleSubstitution(s);
		    checkLocksHeld(ms, expressions,ne.getStartLoc(), "", ne.decl.pmodifiers);
		}
		return e;
	    }
	...
	case TagConstants.CASTEXPR:
		...
		if( !Types.isCastable( exprType, ce.type ) ) {
			...
			if (Types.isCastable(((ArrayType)exprType).elemType,
			    ...
			    ErrorMsg.print("BadCast", 
					   ce.getStartLoc(),
					   "suspicious cast from '" +
					   Types.printName(exprType) +
					   "' to '" +
					   Types.printName(ce.type)+"'", Location.NULL);
			}
			       ...
			       ((rcc.tc.TypeSig)exprType).generic ==
			       ((rcc.tc.TypeSig)ce.type).generic) {
			    ErrorMsg.print("BadCast", 
					   ce.getStartLoc(),
					   "suspicious cast from '" +
					   Types.printName(exprType) +
  ...
  protected Type checkObjectDesignator(Env env, ObjectDesignator od,
	...
	ExprObjectDesignator eod =
	    (ExprObjectDesignator)super.
	    checkObjectDesignator( env, od, staticContext);
	if (eod != null && eod.expr instanceof ThisExpr) {
	    if (((ThisExpr)eod.expr).classPrefix==null) {
		((ThisExpr)eod.expr).classPrefix = sig;
	    }
	}
	return eod;
    ...
    */
	...
	if (!isFinalExpr(env, checkExpr)) {
	    ErrorMsg.print(sig, "ConstantLocks",
			   expr.getStartLoc(),"",
			   assocLoc);
	}
	return checkExpr;
	...
	switch(tag) 
	    ...
	    case TagConstants.REQUIRESMODIFIERPRAGMA:
		{
			...
			ErrorMsg.print(sig, "Modifiers",
				       ctxt.getStartLoc(),"requires modifiers only allowed on methods.",
				       Location.NULL);
			break;
		    }
	    ...
	    case TagConstants.GUARDEDBYMODIFIERPRAGMA:
		{
			...
			ErrorMsg.print(sig, "Modifiers",
				       ctxt.getStartLoc(),"guarded_by modifiers only allowed on fields and classes.",
				       Location.NULL);
			break;
		    }
	    ...
	    case TagConstants.THREADLOCALSTATUSPRAGMA:
		{
			...
			ErrorMsg.print(sig, "ThreadLocal",
				       ctxt.getStartLoc(),"thread_local/thread_shared only allowed on classes and interfaces.",
				       Location.NULL);
			break;
		    }
		    if (threadLocalDecoration.get(ctxt)!=null) {
			ErrorMsg.print(sig, "ThreadLocal",
				       ctxt.getStartLoc(),"only one thread_local/thread_shared modifier allowed per class.",
				       p.getStartLoc());
			break;
			
		    }
		    threadLocalDecoration.set(ctxt,new Boolean(((ThreadLocalStatusPragma)p).local));
		    break;
		}
	    default:
		Assert.fail("Unexpected tag " + tag);
    ...
    protected void checkStmtPragma(Env env, StmtPragma s) {
	inAnnotation = true;	// Must be reset before we exit!
	int tag = s.getTag();
	switch(tag) {
	...
	if (rcc.Main.prg) {
		...
		ErrorMsg.print(sig, "Modifiers",
			       fd.getStartLoc(),"field '"+fd.id+"' has redundant guard annotations.  Guards are: '" + PrettyPrint.inst.toString(g)+"'",
			       Location.NULL);
	    }
	}
     ...
     * a method declared in a superclass.
     **/
	...
	case TagConstants.FIELDACCESS:
		...
		try {
		    FieldDecl fd = Types.lookupField(getObjectDesignatorType(env, fa.od),fa.id, sig);
	...
	case TagConstants.SUPEROBJECTDESIGNATOR:
		...
		TypeDecl d = sig.getTypeDecl();
		TypeSig superSig = 
		    TypeSig.getSig( ((ClassDecl)d).superClass ); //@ nowarn NonNull
		return superSig;
	    }
	...
	if (b == null) {
	    ...
	    a.inAnnotation = true;
	    a.rootSEnv = a.makeEnvForTypeSig(a.sig, true);        //@ nowarn Invariant
	    a.rootIEnv = a.makeEnvForTypeSig(a.sig, false);        //@ nowarn Invariant
...
.getCompilationUnit());
	...
	} else if (rcc.Main.dts && onCommandLine) {
	...
	Info.out("[infering that " + TypeSig.getSig(d).simpleName +
		 " is " +
		 (((Boolean)threadLocalDecoration.get(d)).booleanValue()?
	...
	for (int i = 0; i<d.elems.size(); i++) {
		...
		if (Modifiers.isStatic(fd.modifiers)) {
			...
			if (getLocalThreadStatus(decl, env)) {
			    ErrorMsg.print(sig, "ThreadLocal",
					   fd.getStartLoc(),
					   "the class '" + TypeSig.getSig(d).getExternalName()  + 
					   "' must be thread shared because field '"+ 
					   TypeSig.getSig(d).getExternalName()+"."+ fd.id + 
					   "' is static",
			...
			if (!guardPragmaExists(fd.pmodifiers)) {
			    ErrorMsg.print(sig, "StaticField",
					   fd.getStartLoc(),
					   "field '" + TypeSig.getSig(d).getExternalName()+"."+ fd.id + 
	...
	for (int i = 0; i<d.elems.size(); i++) {
	    ...
	    case TagConstants.METHODDECL:
		{
			...
			if (!getLocalThreadStatus(superDecl.parent,env)) {
			    ErrorMsg.print(sig, "Override",
					   md.getStartLoc(),
					   "'" + md.id.toString() + "'",
					   superDecl.getStartLoc());
			}
		    }
		    break;
		}
	...
	if (d instanceof ClassDecl&&((ClassDecl)d).superClass!=null) {
	    TypeDecl parent = env.resolveTypeName(((ClassDecl)d).superClass).getTypeDecl();
	    if (parent!=null&&getLocalThreadStatus(parent,env)) {
		ErrorMsg.print(sig, "ThreadLocal",
			       d.getStartLoc(),
			       "super class '"+ TypeSig.getSig(parent).getExternalName() + "' of thread shared class '" + TypeSig.getSig(d).getExternalName() + "' must be thread shared",
			       parent.getStartLoc());
	    }
	}
	
	for (int i = 0; i<d.superInterfaces.size(); i++) {
	    TypeDecl p = env.resolveTypeName(d.superInterfaces.elementAt(i)).getTypeDecl();
	    if (getLocalThreadStatus(p,env)) {
		ErrorMsg.print(sig, "ThreadLocal",
			       d.getStartLoc(),
			       "thread shared class " + TypeSig.getSig(d).getExternalName() +" can not implement thread local interface " +  p.id,
	...
	for (int i = 0; i<d.elems.size(); i++) {
		...
		if (!Modifiers.isFinal(fd.modifiers)) {
			...
			if (rcc.Main.agt) {
				...
				Info.out("[assuming the unguarded fields of "+
					 sig+" are guarded by this/" + sig + ".class]");
				unsharedFieldSeen = true;
			    }
			    ExprVec expressions = ExprVec.make();
			    if (Modifiers.isStatic(fd.modifiers)) {
				expressions.addElement(ClassLiteral.make(sig, fd.getStartLoc()));
			    } else {
				expressions.
				    addElement(ThisExpr.
					       make(sig, fd.getStartLoc()));
			    }
			    ...
			    if (!Modifiers.isStatic(fd.modifiers)) {
				ErrorMsg.print(sig, "SharedField",
					       fd.getStartLoc(),
					       "field '" + TypeSig.getSig(d).getExternalName()+"."+ fd.id + "' must be guarded in a thread shared class",
					       Location.NULL);
			    } else {
		...
		if (!arrayGuarded(fd.type)) {
			...
			ErrorMsg.print(sig, "SharedArray",
				       fd.getStartLoc(),
				       "elements of array '" + TypeSig.getSig(d).getExternalName()+"."+ fd.id + "' must be guarded in a thread shared class",
				       Location.NULL);
		    }
		}
		...
		if (type != null) {  // we have a typesig as the type 
			...
			ErrorMsg.print(sig, 
				       ...
				       "ThreadLocal",
				       fd.getStartLoc(),
				       "the class '" + type + "' must be thread shared because field '" + TypeSig.getSig(d).getExternalName()+"."+ fd.id + "' belongs to a shared class",
				       threadLocalAnnotationLoc(decl));
		    }
	...
	if (d == Types.getJavaLang("Thread").getTypeDecl() ||
	    Types.isSubclassOf(TypeSig.getSig(d),
  ...
  /** A precondition of this method is that <code>staticContext</code> be
    * set appropriately.
    **/
  ...
  protected Expr checkDesignator(Env env, Expr e) {
	  ...
	  case TagConstants.FIELDACCESS:
		  ...
		  FieldAccess fa = (FieldAccess)e;
		  try {
		      FieldDecl fd = Types.lookupField(getObjectDesignatorType(env, fa.od),
						       fa.id, sig);
