...
public class EqualsASTNoDecl extends EqualityVisitorSuper {
    ...
    public Object prep(ASTNode y, Object o) {
        if (y instanceof CastExpr) { return new Boolean(equals(
            ((CastExpr) y).expr,
            (ASTNode) o)); }
        if (y instanceof ParenExpr) { return new Boolean(equals(
            ((ParenExpr) y).expr,
            (ASTNode) o)); }
        if (y instanceof CastExpr) { return new Boolean(equals(
            y,
    ...
    public boolean equals(ExprVec a, ExprVec b) {
        ...
        for (int i = 0; i < a.size(); i++) {
            if (!equals(a.elementAt(i), b.elementAt(i))) { return false;
    ...
    public boolean contains(ExprVec a, Expr expr) {
        for (int j = 0; j < a.size(); j++) {
            if (equals(a.elementAt(j), expr)) { return true; }
        }
        return false;
    }
