...
public class Main extends javafe.SrcTool
{
	...
	*/
	...
	if(options().nvcg){
	    ...
	    VcGenerator vcg = new VcGenerator(vcBody, options().pErr, options().pWarn, options().pInfo, options().pColors);
		...
		try {
		    ...
		    fw.write(vcg.pvsProof(label));
		
		    fw.close();

		    System.out.println("[Pvs many sorted proof using the new vcg has bee written to "+fn+".]");
		}
		catch (Exception e) { 
		    System.out.println(e.getMessage()); 
		}
