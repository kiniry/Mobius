...
public class Main extends javafe.SrcTool
{
	...
	*/
	...
	if(options().nvcg){
	    ...
	    VcGenerator vcg = new VcGenerator(new SimplifyProver(), vcBody, options().pErr, options().pWarn, options().pInfo, options().pColors);
		...
		try {
		    ...
		    fw.write(vcg.getProof());
		
		    fw.close();

		    System.out.println("[Simplify proof using the new vcg has been written to "+fn+"]");
		}
		catch (Exception e) { 
	    ...
	    if(options().pPvs){
		try {
			vcg = new VcGenerator(new PvsProver(), vcBody, options().pErr, options().pWarn, options().pInfo, options().pColors);
	    ...
	    if(options().pCoq){
			try {
				// to have the same proofs as the old vc tree...
			    Expr vc = GC.implies(initState.getInitialState(), vcBody);
				vcg = new VcGenerator(new CoqProver(), vc, options().pErr, options().pWarn, options().pInfo, options().pColors);
...
// 		    }
		    ...
		    System.out.println("[Graph of generic proof have been written to "+fn+".ps]");
