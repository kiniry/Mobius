	...
	*/
	...
	Expr vc = GC.implies(initState.getInitialState(), vcBody);
    	...
    	if (options().tvcg) {
    		...
    		Lifter l = new Lifter(vc);
    		l.run();
    		SimplifyNodeBuilder simpl = new SimplifyNodeBuilder();
    		String cond = simpl.dump(l.build(simpl));
    		...
    		try {
    			...
    			fw.write(cond);
    			fw.close();
    			System.out.println("[NEW VC " + method + " written to " + fn + "]");
    		} catch (IOException ex) {
    			ErrorSet.fatal("fatal error " + ex.getMessage());
    		}
