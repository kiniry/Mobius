...
public class Options extends javafe.SrcToolOptions
{
  ...
  final String[][] escPrivateOptionData = {
    ...
    { "-pSimplify",
      "Write the vc for simplify generated by the new verification condition generator to a file."},
    { "-vc2dot",
  ...
  public boolean ifpvc2dot = false;
  //$$
  ...
  public int processOption(String option, String[] args, int offset) 
    ...
    else if (option.equals("-ifpvc2dot")){
      ifpvc2dot = true;
	...
	while(!parsingFinished) { 
	  ...
	  if( (index = s.lastIndexOf(',')) == -1) { // the last parameter
	    parsingFinished = true;
	    sTemp = s;
	  }
	  else {
	    /*
	     *  Example on 'time,space'
	     */
	    index+=1;
	    sTemp = s.substring(index, s.length()); // sTemp = space
	    index-=1;
	    s = s.substring(0,index); // s = time
	  }

	  //@ assert s.length() != 0 & sTemp.length() != 0;
