...
\item protected void setType(TypeInfo type, boolean sure) : this function is used to set the type of a node. For example, if we are in a node representing the addition of 2 \textit{integers}, we can deduce that all sons should have type \textit{integer} too. Thus, in such a case, you will write code looking like :
...
\paragraph{Class extending TFunction}{}
...
I won't explain, but if you have a good knowledge of Simplify's logic, it will makes sense (you can read \textit{The logic and calculi of ESC/Java2}, that's explained), so the additional hypothesis are :
\begin{verbatim}
      (AND 
        (EQ |elems@pre| elems)
        (EQ elems 
