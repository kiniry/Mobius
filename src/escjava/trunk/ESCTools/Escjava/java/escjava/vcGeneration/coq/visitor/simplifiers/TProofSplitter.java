...
public class TProofSplitter extends ATSimplifier {
	/** the list of terms that were generated by this splitter */
	private /*@ non_null @*/ List listTerms = new ArrayList();
	
	/** tells how much the splitter shall go recursively over the <code>or</code>s */
	private int morbidity = 5;
		...
		if(noddor instanceof TBoolOr) {
			...
			if (!(n.parent.parent == null)) {
				while(iter.hasNext()) {
					...
					TProofSplitter tps = new TProofSplitter();
					tps.morbidity = this.morbidity - 1;
					nod.accept(tps);
					if(tps.listTerms.size() == 0) {
						listTerms.add(nod);
					}
					else {
						listTerms.addAll(tps.listTerms);
					}
				}
				return;
			}
	...
	/**
	 * Returns the list of proof obligations that
	 * were generated by the splitter.
	 * @return A list of {@link TNode}
	 */
	public List getListTerms() {
		return listTerms;
	}
