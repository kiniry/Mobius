...
<code>CompilationUnit</code>-level type checks.

<p> In practice, these are mostly checks that the set of import
declarations is legal.
...
public class CheckCompilationUnit {
	...
	/*
	 * Check to make sure that each import is individually well
	 * formed:
	 */
	checkImports(cu);
	...
	/*
	 * Check for "import P.T" and "import Q.T" pairs.  Such a pair
         * is legal iff P=Q.
         *
	 * Moreover, check for "import P.T" where T is defined in this
	 * CompilationUnit.  Such imports are legal iff P is the name of
	 * this package.
	 */
	for (int i = 0; i < imports.size(); i++) {
	    ...
	    // Get the full name and unqualified name of the type it's
	    // importing:
	    ...
	    /*
	     * Check to see if N1 is of the form P.T where T is declared
	     * in this CompilationUnit and P is not this
	     * CompilationUnit's package:
	     */
  ...
  // Private methods
    ...
    /** Check a <code>CompilationUnit<code> to make sure that the only
	type that may be declared public in it is the one with the same
	name as the file it occurs in.  Violations result in warnings
	only. */
