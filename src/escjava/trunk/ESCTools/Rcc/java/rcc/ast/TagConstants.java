...
public class TagConstants extends GeneratedTags {
    ...
    public static int checkFromString(String s) {
        for (int i = FIRSTRCCCHECKTAG; i <= LASTRCCCHECKTAG; i++) {
            if (s.equals(rccchecks[i - FIRSTRCCCHECKTAG])) return i;
        }
        // @ unreachable
    ...
    private static Identifier[] rcckeywords = {
        Identifier.intern("guarded_by"), Identifier.intern("requires"),
        Identifier.intern("holds"), Identifier.intern("no_warn"),
        Identifier.intern("elems_guarded_by"),
        Identifier.intern("thread_local"), Identifier.intern("thread_shared"),
        Identifier.intern("readonly"), Identifier.intern("ghost") };

    private static String[] rccchecks = {
        "BadCast", "ConstantLocks", "Modifiers", "Super", "Race",
        "ThreadLocal", "Override", "Message", "ReadOnly", "StaticField",
        "SharedField", "SharedArray" };
