...
/**
 * NOTE: According to the documentation of <code>javafe.tc.TypeSig</code>
 * this class, which is responsible of typechecking at signature level,
 * meaning that it does not look at fields of types. In order to avoid
 * call cycles it shoud never use functions from 
 * <code>FlowInsensitiveChecks</code> which takes care of the last phases
 * of typechecking. 
 */
// TODO: Comment this!
public class PrepTypeDeclaration extends javafe.tc.PrepTypeDeclaration {
    ...
    public void visitInterfaceDecl(InterfaceDecl decl,
        ...
        checkTypeModifierPragmaVec(decl.tmodifiers, decl, getEnvForCurrentSig(
                currentSig, true), currentSig);

        // Visit all enclosed member declarations
        // They will add themselves to fieldSeq and methodSeq

        for (int i = 0; i < decl.elems.size(); i++)
            visitTypeDeclElem(decl.elems.elementAt(i), currentSig, true, false,
                    true);
    ...
    protected void checkTypeModifierPragmaVec(TypeModifierPragmaVec v,
            ASTNode ctxt, Env env, javafe.tc.TypeSig currentSig) {
        if (v != null)
            for (int i = 0; i < v.size(); i++)
    ...
    /**
     * Transforms ghost annotations into field declarations and wraps
     * them in <code>GhostDeclPragma</code> nodes.
     */
    ...
    protected void checkTypeModifierPragma(
        ...
        case TagConstants.GENERICPARAMETERPRAGMA: {
            ...
            for (int i = 0; i < pp.args.size(); i++) {
                ...
                GhostDeclPragma pragma = GhostDeclPragma.make(decl, parameter.locId);
    ...
    /**
     * Look up in <code>instantiations</code> one that matches <code>sig</code>
     * instantiated with arguments <code>expressions</code>.
    ...
    protected void processGenericArgumentPragmas(Env env, TypeName tn) {
        ...
        for (int i = 0; i < tn.tmodifiers.size(); i++) {
            ...
            case TagConstants.GENERICARGUMENTPRAGMA:
                 ...
                 */
                javafe.tc.TypeSig sigForEnclosingClass = env
                        .getEnclosingClass();
                FlowInsensitiveChecks fic = new FlowInsensitiveChecks(
                        sigForEnclosingClass, getEnvForCurrentSig(
                                sigForEnclosingClass, false));
...

                {
                    if (sigForEnclosingClass.state < TypeSig.PREPPED) {
                        rcc.tc.TypeSig.prepPartDecoration.set(
                                sigForEnclosingClass, new PrepPart(
                                        getFieldsFromStack(),
                                        getMethodsFromStack()));
                    }
                    ...
                    rcc.tc.TypeSig.prepPartDecoration.set(sigForEnclosingClass,
                            null);
                }
    ...
    public javafe.tc.TypeSig processTypeNameAnnotations(
        ...
        if (env.getEnclosingClass() == null) {
            ...
            // TODO: "_dummy" should be a constant, which also can't be produced by parser
     ...
     * 
     * In the process, detect if we don't provide any argument to a
     * templete type.
     * 
    ...
    public javafe.tc.TypeSig findTypeSignature(
        ...
        if (expressions == null) {
            if (typeParametersDecoration.get(sig) != null) {
                ...
                ErrorSet.fatal(
                    locForError, 
                    "no type arguments given for " + sig.simpleName
                );
            } else {
     ...
     * <code>expressions</code>. Note that the resulting signature
     * has not been prepped (it lacks information about its methods
     * and fields).
     * 
     * A bunch of errors (such as templete parameter mismatch) is
     * detected and reported during the process.
     *  
     ...
     * @return A new signature in a new environment pointing to a
     *      clone declaration that has template arguments substituted
    ...
    protected javafe.tc.TypeSig createInstantiation(
        ...
        FormalParaDeclVec parameters = (FormalParaDeclVec) 
            PrepTypeDeclaration.typeParametersDecoration.get(sig);
        if (parameters == null) {
            Env currEnv = getEnvForCurrentSig(sig, true);
            checkTypeModifierPragmaVec(decl.tmodifiers, decl, currEnv, sig);
            parameters = (FormalParaDeclVec) 
                PrepTypeDeclaration.typeParametersDecoration.get(sig);
            if (parameters == null) {
                ErrorSet.fatal(
                    decl.getStartLoc(), 
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            subs.addElement(new Substitution(AmbiguousVariableAccess.make(
                SimpleName.make(
                    parameter.id, 
                    parameter.getStartLoc())
                ),
    ...
    protected EnvForTypeSig getEnvForCurrentSig(javafe.tc.TypeSig sig,
            boolean isStatic) {
        Env env;
        ...
        EnvForInstantiation envForCheck = new EnvForInstantiation(env, sig,
                getFieldsFromStack(), getMethodsFromStack(), isStatic);
        // envForCheck.display();
        return envForCheck;
    }
