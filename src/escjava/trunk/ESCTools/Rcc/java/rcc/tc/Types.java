...
public class Types extends javafe.tc.Types {
    ...
    protected javafe.tc.TypeSig makeTypeSigInstance(String[] packageName,
						    ...
						    TypeDecl decl,
						    CompilationUnit CU) {
	return new rcc.tc.TypeSig(packageName,
				  simpleName,
				  enclosingType,
				  decl,
				  CU);
    ...
    */
	...
	case TagConstants.ARRAYTYPE:
		...
		equality.equalsSet(getElemGuard((ArrayType)x),  getElemGuard((ArrayType)y)); //@nowarn Cast
	...
	if( s instanceof PrimitiveType ) 
		...
		if( t instanceof PrimitiveType ) {
		    return javafe.tc.Types.isAnyPrimitiveConvertable( (PrimitiveType)s, (PrimitiveType)t );
		}
		...
		if( t instanceof javafe.tc.TypeSig ) 
			...
			if( sElem instanceof PrimitiveType 
				...
				return sElem.getTag() == tElem.getTag()&&
				    equality.subset(getElemGuard((ArrayType)s),  getElemGuard((ArrayType)t));
			    }
	...
	Assert.notNull(s);		//@ nowarn Pre
	Assert.notNull(t);		//@ nowarn Pre
	if(s instanceof javafe.tc.TypeSig 
	   && t instanceof javafe.tc.TypeSig 
	...
	if( s.getTag() == TagConstants.ARRAYTYPE ) {
		...
		return (isSameType( sElem, tElem )
			|| javafe.tc.Types.isWideningReferenceConvertable(sElem,tElem))&&
		    equality.subset(getElemGuard((ArrayType)s),  getElemGuard((ArrayType)t));
	    }
     ...
     **/
    //@ requires t!=null
    protected FieldDecl lookupFieldInstance(Type t, Identifier id, javafe.tc.TypeSig caller) 
