...
/*
 * Change history: 28 Sep 2000 flanagan Created
 */
...
public class AnnLinks extends javafe.SrcTool {
    ...
    /**
     * * Returns the pretty printer to set * <code>PrettyPrint.inst</code> to.
     */
    ...
    /**
     * * Called to obtain an instance of the javafe.tc.TypeCheck class * (or a
     * subclass thereof). May not return <code>null</code>. By * default,
     * returns <code>javafe.tc.TypeCheck</code>.
     */
     ...
     * <code>args</code>.
     * <p> * * This is the main entry point for the <code>rccjava</code> *
     * command.
     * <p>
     */
...
/**
 * this visitor writes to stdout a table linking error locations to decl
 * locations, and decl locations to corresponding non_null annotations. Just
 * does non_null on fields and parameters, and links from assignment statements
 * to decl for rhs.
 */
...
class AnnLinkVisitor extends DefaultVisitor {
    ...
    public void visitModifierPragma(ModifierPragma x) {
        ...
        System.out.println("AnnText " + convert(x.getStartLoc()) + " "
            + baos.toString());
    }
    ...
    public void visitTypeDecl(TypeDecl x) {
        ...
        System.out.println("DeclName " + convert(x.getStartLoc()) + " type "
            + x.id);
    }
    ...
    private ModifierPragma searchModifierPragmas(
        ModifierPragmaVec x,
        String contains) {
        if (x != null) {
            for (int i = 0; i < x.size(); i++) {
                ...
                if (baos.toString().indexOf(contains) >= 0) return mp;
            }
        }
        return null;
    }
    ...
    private void reportThreadLocalModifier(TypeDecl x) {
        if (x.pmodifiers != null) for (int i = 0; i < x.pmodifiers.size(); i++)
    ...
    private void reportWarnDecl(int locdecl, int locann) {
        if (locdecl != Location.NULL && !Location.isWholeFileLoc(locdecl)
            ...
            System.out.println("WarnDecl " + convert(locdecl) + " "
                + convert(locann));
        }
    }
    ...
    private void reportDeclAnn(int locdecl, int locann) {
        if (locdecl != Location.NULL && !Location.isWholeFileLoc(locdecl)
            && locann != Location.NULL && !Location.isWholeFileLoc(locann)) {
            System.out.println("DeclAnn " + convert(locdecl) + " "
                + convert(locann));
        }
    }

    private String convert(int loc) {
        return Location.toFileName(loc) + " " + Location.toLineNumber(loc)
