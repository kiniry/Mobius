...
public class AnnotationVisitor extends DefaultVisitor {
    ...
    public void visitRoutineDecl(RoutineDecl rd) {
        ...
        Annotator requiresAnnotator = new Annotator(rd, "parameter:" + ms,
                "requires ");

        boolean isStatic = Modifiers.isStatic(rd.modifiers)
                || rd instanceof ConstructorDecl;
        // @ assume rd.parent != null;
        Hashtable envReq = getEnv(rd.parent, !isStatic,
                accessLevel(rd.modifiers), true, false, null);
    ...
    public void visitFieldDecl(FieldDecl fd) {
        ...
        Hashtable env = getEnv(fd.parent, !Modifiers.isStatic(fd.modifiers), 0,
                !isConstantFinal(fd), false, null);
    ...
    private Hashtable getEnv(/* @ non_null */TypeDecl td,
            boolean allowInstance, int accessLevel,
            boolean allowConstantFinals, boolean returnNonConstantNames,
            FieldDecl fdUpto) {
        ...
        FieldDeclVec fds = sig.getFields(true); // rgrig: include inherited. TODO: check
        ...
        for (int i = 0; i < fds.size(); i++) {
            ...
            if (!TypeCheck.inst.canAccess(sig, fdsig, fd.modifiers,
                    fd.pmodifiers))
