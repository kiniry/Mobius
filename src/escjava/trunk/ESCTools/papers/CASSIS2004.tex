...
(\cite{Leavens-etal00} and various papers listed at \cite{jmlpapers}), so here we will give just one example showing some of the syntax.  The class in Fig.~\ref{fig:example}
...
\item \texttt{spec\_public} indicates that the field named {\em seq} has public visibility in specifications.
\item The \texttt{invariant} states that after construction {\em seq} is never null and is an array with \texttt{Object}s as elements.
...
\begin{itemize}
\item The default constructor does not set the value of \texttt{seq} to a non null array as required
by the invariant.
...
assignments of Object references to its elements.
\end{itemize} 
...
JML, but not ESC/Java, allows pure method and constructor calls (that is, methods
...
first-order logic, the result of a function in Simplify depends only on its
...
\paragraph*{} The Simplify theorem prover used by both ESC/Java and ESC/Java2 remains unchanged,
...
% Add discussion of Simplify and moves on that front.   TODO
\end{itemize}
...
% explanation.
%     For many readers, this will be a problem.  So a short 
% introduction
% (1 page max)
%     should be added, giving an example of a very small JML class 
% spec,
% and introducing
%     the terminology: ensures, requires, invariant, model, datagroups, 
% represents etc.
...
% (especially)
%     the solutions that were taken, are too terse or missing.  A 
% little more
%     detail is needed in places like:

%    Last sentence of Sect 2.2 says "virtually all of the diffs betwee 
% JML
% and ...
%    have been resolved."   -- explain which ones have not, or at least 
% say
%    how many have not and why.
...
% it
% unclear
%    how much of this is currently supported.
...
% separately.
%    The two sentences about dynamic allocations are unclear, so need 
% rewriting.

Revised and expanded a bit.

%    p5: "This also changed the usage and semantics of the also 
% keyword."
%    Explain!
...
% a
% [pure?]
%        method in Java and the concept of a function in the prover."
%       (It was unclear if you are talking of arbitrary Java methods, 
% or
% just the pure ones).

Clarified.

% p4: "as functions of the object that owns them and the global state."
%       (Why the global state as well?)
...
% more
%        positive based on the results of your case study? 
...
Some clarification added, though a full treatment would take a lot more space
...
A paragraph added abot Simplify.
...
% mouthful, and
%   after a while, becomes distracting.  It would be better to simply 
% use
%   "ESC/Java" (or "SRC ESC/Java" if you are concerned about the 
% similarity
%   of the names).
...
% successor to
%   ESC/Modula-3 (and give a reference to the ESC/Modula-3 tech report).
...
% to
%   "issues with" Simplify.  Could you elaborate on this, maybe by 
% listing a
%   few of these issues?
...
% Author: David R. Cok1 and Joseph R. Kiniry
...
TODO _ Missing the reference for ESCModula-3
TODO Missing the reference for  PruningPaper
