...
programming productivity.  Appendices list the details of the enhancements to
...
Creating ESC/Java2 required changes to DEC/SRC
...
and those sets may be extended by subtypes.
 Each (or poossibly multiple) specification cases of a routine may be guarded by a precondition and may specify a
...
thus may have unbounded size (consider the datagroup of all of the `next' fields of a 
LinkedList).  ESC/Java2 currently deals with this by unrolling the recursion to a fixed
...
ESC/Java2 supports the JML syntax and also performs static checking.
...
heap as an additional parameter, and (c) incorporate the specifications of the called
method as additional axioms.  

Dynamic allocations of objects using constructors can be recast as method calls and treated as above.
...
Model fields together with \texttt{represents} clauses
provide a substantial benefit in abstraction, especially since the
...
in static checkers.
\end{itemize}
There are other, less significant issues concerning helper annotations, the \texttt{measured\_by},
\texttt{hence\_by}, \texttt{weakly}, \texttt{accessible} and \texttt{callable} clauses, and
model programs. 
...
of the \htmladdnormallinkfoot{http://www.verificard.org/}{European VerifiCard
...
ESC/Java2 statically checks specifications;
...
\section{Principal changes implemented in ESC/Java2}
The Java Modeling Language is a research project in itself; hence the
JML syntax and semantics are evolving and are somewhat of a moving
target (and there is as yet no complete reference manual).  However,
the core language is reasonably stable.  The following are key
