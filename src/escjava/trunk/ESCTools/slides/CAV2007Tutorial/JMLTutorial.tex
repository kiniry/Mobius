...
\lstinline!a! instaed of making it \lstinline!nullable!.
}
\end{frame}
...
\begin{frame}[fragile]
\frametitle{Formal Notation for Method Specification}
...
\end{itemize}
\includegraphics[height=2in]{multiple-inh}
\end{frame}

\begin{frame}
\frametitle{Effective Specification for Animals}
\framesubtitle{Inherits All Spec Cases}

\lstinputlisting[linerange={20-27}]{examples/Animal.java}
\end{frame}
...
\lstinputlisting[linerange={1-1,31-40}]{examples/Patient.java}
\end{frame}
...
\begin{question}
What is the effective specification of \texttt{Patient}'s
\texttt{setAge} method?
\end{question}
...
\item[History Constraint:]
\begin{displaymath}
\exthc^T = \bigwedge \{\addedhc^U \mid U \in \supers(T)\}
\end{displaymath}

\item[Initially Predicate:]
...
\begin{frame}[fragile]
\frametitle{Case Analysis + Supertype Abstraction}
\lstinputlisting[linerange={2-12}]{examples/HeadTest.java}
\end{frame}
...
\begin{itemize}
\item
Comparison to similar Spec\# methodology.
...
\begin{frame}
\frametitle{Acknowldegments}
Thanks to David Naumann, 
William Weihl, Krishna Kishore Dhara, 
Cesare Tinelli, Don Pigiozzi, Barbara Liskov, Jeannette Wing,
Yoonsik Cheon, Al Baker,  
Clyde Ruby, Tim Wahls, Patrice Chalin, 
Curtis Clifton, David Cok, Joseph Kiniry, 
Rustan Leino, Peter M\"{u}ller, 
...
\begin{lstlisting}
  ...
  //@ assert p.log.size() == 0;
\end{lstlisting}
\note{This works because the intially clause of Patient also applies
  to FemalePatient.
}
\end{frame}
