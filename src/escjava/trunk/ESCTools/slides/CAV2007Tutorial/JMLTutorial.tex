...
The elems field is non-null (the default) and the elements are allowed
to be null.
}
\end{frame}
...
\begin{lstlisting}[mathescape=true]
$T$ $o$ = createNewObject();
//@ assume $o$.$\extinit^T$ && $o$.$\extinv^T$;

/* ... */

//@ assert $o.\extpre^T_m$;
$o$.$m$();
//@ assume $o.\extpost^T_m$;
//@ assume $o.\extinv^T_m$ && $o$.$\exthc^T$;
\end{lstlisting}
\note{Here $T$ is the static type of $o$, that's why all the
...
//@ assert $o$.$\extinit^{T'}$ && $o$.$\extinv^{T'}$;

/* ... */

//@ assume $o.\extpre^{T'}_m$;
$o$.$m$();
//@ assert $o.\extpost^{T'}_m$;
//@ assert $o.\extinv^{T'}_m$ && $o$.$\exthc^{T'}$;
\end{lstlisting}
\note{Here $T$ is the static type of $o$, 
     but it's dynamic type is $T'$.
}
\end{frame}
...
\begin{definition}
Suppose $T' \STO T$.  Then \\
$T'$ \emph{is a strong behavioral subtype of} $T$
if and only if:
