...
\figref{Example:TickTockClock}. This method may modify the three private 
...
%
A pure method (\secref{Sec:purity}) is a method with a frame condition
\texttt{assignable \nothing}.
...
\secref{Sec:behsubtyping}, the implementation of \texttt{tick()} in any
...
The specification in \figref{Example:TickTockClock} declares a (public) datagroup
...
% PC: why the bad press?
% \footnote{These are a part of the (incomplete) JML specifications developed
%  for the Java API that are distributed along with some the tools.}
%
...
\begin{figure}[tbp] 
%
\VerbatimInput[numbers=left]{Clock.java}
%
\vspace*{-2ex} % to fit this on one page
...
to this. --Erik  Don't think we'll have time to add that. --P}
...
%
The specification in \figref{Example:model} illustrates the use of a model field.
...
(a subclass of \texttt{Clock}, given in \figref{Example:alarmclock}) uses 
...
%
% Erik: I removed the sentence below, as overriding represents clause
%       is a dubious thing to do.
% Similarly, a subclass
...
For example, consider the class \texttt{AlarmClock} in \figref{Example:alarmclock}.
...
\begin{figure}[tbp] 
%
\VerbatimInput[numbers=left]{AlarmClock.java}
%
\vspace*{-2ex} % to fit this on one page
...
%
\VerbatimInput[numbers=left]{AlarmInterface.java}
%
\vspace*{-2ex} % to fit this on one page
...
% \end{figure}
% 
% \subsection{Example ghost field: a boolean \texttt{\_alarmEnabled}}
...
%
In \figref{Example:alarmclock}, we have declared 
...
% %
% \caption{\label{example}Example JML specification using ghost field \texttt{\_alarmEnabled} to record if alarm is going off}
% \label{ghosttwo}
% \end{figure}
...
% \MARGIN{I think the model field is clearer :) --PC}
%
...
%
\VerbatimInput[numbers=left]{DigitalDisplayClock.java}
%
...
%
\VerbatimInput[numbers=left]{BrokenDigitalDisplayClock.java}
%
\vspace*{-2ex} % to fit this on one page
...
To illustrate the issue, \figref{Example:alias} shows
...
\figref{Example:alias}, the \texttt{time} array is declared
...
in \figref{Example:brokenalias} is not well-typed because it breaks 
