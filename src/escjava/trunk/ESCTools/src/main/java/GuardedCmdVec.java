/**
 ** GuardedCmdVec: A Vector of GuardedCmd's. <p>
 **
 ...
 ** $Id: GuardedCmdVec.j 1928 2006-05-19 12:43:10Z chalin $
 **
 ** DO NOT EDIT!!<p>
 **/

/*
 * File: GuardedCmdVec.java <p>
 *
 ...
 *
 * GuardedCmd should be globally replaced with the simple name of the
 *		element type<p>
 ...
 *
 * The resulting type will have name GuardedCmdVec.<p>
 * 
 ...
 *
 *
 * By Default, GuardedCmdVec's may not contain nulls.  If the ability to
...
public class  GuardedCmdVec {
    ...
    private /*@ spec_public non_null*/ GuardedCmd[] elements;
    //@ invariant (\forall int i; (0<=i && i<count) ==> elements[i]!=null);
    //@ invariant \typeof(elements) == \type(GuardedCmd[]);
	...
	this.elements = new GuardedCmd[count];
	...
	this.elements = new GuardedCmd[(cnt == 0 ? 2 : cnt)];
	//@ set elements.owner = this;
	...
	return new GuardedCmdVec(count);
    }

    //@ requires vec!=null;
    //@ requires vec.elementType <: \type(GuardedCmd);
    //@ requires !vec.containsNull;
    //@ ensures \result!=null;
    //@ ensures \fresh(\result);
    public static GuardedCmdVec make(Vector vec) {
	int sz = vec.size();
	GuardedCmdVec result = new GuardedCmdVec(sz);
	result.count = sz;
	...
	return new GuardedCmdVec(els);
    }

    //
    //@ requires s.vectorCount>1;
    //@ requires s.elementType <: \type(GuardedCmd);
    //@ ensures \result!=null;
    ...
    public static GuardedCmdVec popFromStackVector(/*@non_null*/ StackVector s) {
	// Creates a new GuardedCmdVec from top stuff in StackVector
	int sz = s.size();
	GuardedCmdVec r = new GuardedCmdVec(sz);
  ...
  public final GuardedCmd[] toArray() {
      int ct = count;
      GuardedCmd[] b = new GuardedCmd[ ct ];
      for(int i = 0; i < ct; i++) {
	  b[i]=elements[i];
      }
      return b;
  ...
  public final GuardedCmdVec copy() {
    GuardedCmdVec result = new GuardedCmdVec(count);
    result.count = count;
  ...
  public final void addElement(GuardedCmd x) {
    if( count == elements.length ) {
      GuardedCmd[] newElements = new GuardedCmd[ 2*(elements.length==0 ?
					      2 : elements.length) ];
  ...
  public final boolean removeElement(GuardedCmd x) {
    ...
    public final GuardedCmd pop() {
        count--;
        GuardedCmd result = elements[count];
	elements[count] = null;
	return result;
    }
