...
// 	}
    ...
    DebugAssert(false, "more than three vars");
  }
  return assert;
}
...
Theorem TheoryQuant::theoryPreprocess(const Expr& e){
  ...
  for(size_t i = 0; i < assertList.size() ; i++){
    new_asserts.push_back(comp_inst_proc.inst(assertList[i]));
...
TheoryQuant::print(ExprStream& os, const Expr& e) {
  ...
  case PRESENTATION_LANG: {
	...
	if(*d_translate || true){
      ...
      const vector<Expr>& triggers = e.getTrigs();
      for (vector<Expr>::const_iterator i=triggers.begin(), iend=triggers.end(); i != iend; ++i) {
