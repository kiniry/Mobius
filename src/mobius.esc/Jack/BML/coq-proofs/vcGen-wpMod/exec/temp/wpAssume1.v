...
| wpIfAss: forall pT PT pF PF PTS PFS post cond thenSt elseSt  substState substStateAux  z, 
                    ...
                    ( pogSubst  PT   substState substStateAux PTS  ) -> 
                    ( pogSubst  PF  substState substStateAux PFS  )  -> 
                    (wpAssume (IfAssume z ( substState, substStateAux)  cond thenSt elseSt )  
                                         post 
                                         ((fun _ => p_and ( p_implies (p_neq (neval substState  substStateAux cond) 0) ( pT  ( substState , substStateAux)) ) 
                                                                       ( p_implies (p_eq   (neval substState substStateAux cond)  0) (pF    ( substState , substStateAux)))) )
                                         (PTS ++ PFS ) )
