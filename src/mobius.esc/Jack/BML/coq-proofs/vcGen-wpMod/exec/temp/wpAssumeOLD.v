...
| exectBsIfTrue: forall s s' cond substState stThen stElse sAux z, 
           neval s sAux cond <> 0 -> 
           execStmtAss s stThen s'  ->
           execStmtAss s (IfAssume z substState  cond stThen stElse) s'
| exectBsIfFalse: forall s s' substState cond stThen stElse sAux z , 
           neval s sAux cond = 0 -> 
...
| exectBsIfTrueAux: forall auxState s s' cond substState stThen stElse sAux z zThen , 
           neval s sAux cond <> 0 -> 
           execStmtAux auxState  s stThen s'  ->
           (isStmtInd (IfAssume z substState cond stThen stElse)  z) ->
           ...
           execStmtAux auxState s (IfAssume z substState  cond stThen stElse) s'
| exectBsIfFalseAux: forall auxState s s' substState cond stThen stElse sAux zElse z , 
           neval s sAux cond = 0 -> 
...
Fixpoint pogSubst   ( vcs : list Assertion )(z : Z )(* State -> AuxState ->*) {struct vcs} : list Assertion   :=
match vcs with 
 | nil => nil 
 |  head::tail => let headS  :=  
                                 (fun s  : State * AuxState => let  (  st ,stAux) := s in
                                             p_implies 
                                                 ( p_forallv  (fun  v =>  p_eq (st v)   ( stAux (AuxName  v z) )   ))  
                                                 (head ( st, stAux)) )  in
                            let tailS := (pogSubst tail z ) in
                            (headS :: tailS  )
end.
...
| wpIfAss: forall pT PT pF PF  post cond thenSt elseSt  substState substStateAux  z, 
                    ( wpAssume thenSt post pT PT) -> 
                    ( wpAssume elseSt post pF PF) ->
                    (*substitute the program variables with the auxiliary variable corresponding to this program point *)
                   (*  ( pogSubst  PT   z  substState substStateAux PTS  ) -> *) 
                     (* ( pogSubst  PF   z substState substStateAux PFS  ) ->  *)  
                    (wpAssume (IfAssume z ( substState, substStateAux)  cond thenSt elseSt )  
                                         post 
                                         ((fun s  => let (st, stAux) := s in
                                                          p_and ( p_forallv  (fun  v =>  p_eq  ( stAux (AuxName  v z) )  (st v)   ))
                                                            ( p_implies ( p_forallv  (fun  v =>  p_eq  ( stAux (AuxName  v z) ) (st v)     ))
                                                                ( p_and 
                                                                      ( p_implies (p_neq (neval st stAux  cond) 0) ( pT  ( st , stAux )) ) 
                                                                      ( p_implies (p_eq   (neval st stAux cond)  0) (pF    ( st , stAux )))))))
                                         ( ( pogSubst  (PT    ++   PF)   z   ) ) )

(*TODO ; apply the substitutions from listModSubst on  Cs Ci Pl *)
| wpWhileAss : forall stmt inv pI PL cond  substState substStateAux (post  : Assertion)  z,
                 ( wpAssume stmt inv pI PL) -> 
                 let Cs := fun s : State * AuxState =>  let (st, stAux ) := s in 
                                       p_and ( p_forallv  (fun  v =>  p_eq (st v)   ( stAux (AuxName  v z) )   ))
                                           ( p_implies ( p_forallv  (fun  v =>  p_eq (st v)   ( stAux (AuxName  v z) )   ))
                                                 (p_implies (p_and  (inv (st, stAux ) )  
                                                                                   (p_neq (neval st stAux cond) 0))  
                                                                    (pI (st, stAux )) ) ) in
                 let Ci := fun s : State * AuxState =>  let (st, stAux ) := s in 
                                        p_and ( p_forallv  (fun  v =>  p_eq (st v)   ( stAux (AuxName  v z) )   ))
                                          (  p_implies ( p_forallv  (fun  v =>  p_eq (st v)   ( stAux (AuxName  v z) )   ))
                                                 (p_implies (p_and (inv (st, stAux ) ) 
                                                                                  (p_eq (neval st stAux cond)  0)) 
                                                                    (post  (st, stAux )) ) ) in
              (*     (pogSubst PL substState substStateAux z  PLS ) -> *) 
                ( wpAssume
                        (WhileAssume z  (inv_j inv)  (substState, substStateAux) cond stmt )  
                        post 
                        inv  
                        ( Cs :: Ci ::   (pogSubst PL  z  ) ) ).

(*states that if every  prop in  a list  holds where all have a hypothesis H and if H holds 
then every prop without H  still holds *)
(* TODO *)
Axiom auxLemma : forall  vcList vcListOneHypMore z ,
 forall  state auxState , 
   vcListOneHypMore  = ( pogSubst vcList z)  ->
   ( forall var ,     ( auxState  (AuxName  var z) ) = (state var) ) ->
   ( forall vc , (In vc vcListOneHypMore  ) -> evalMyProp ( vc (state , auxState )  ) )  ->
   ( forall vc , (In vc  vcList   ) -> evalMyProp  ( vc (state , auxState )  ) )  .
