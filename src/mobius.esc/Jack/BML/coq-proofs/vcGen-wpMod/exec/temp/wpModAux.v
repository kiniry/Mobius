...
Axiom extensionality : forall (A B:Set) (f1 f2:A->B), (forall x, f1 x = f2 x) -> f1 = f2.
...
Definition stateAtLoop (state : State) (stateAux : AuxState )( z: Z) (lVar : list Var )  : State := 
 ...
 (* freezes the values of the variables at a particular program state to the value of a fresh aux variable *)
 Definition FreezeVarAtState (state : State) (stateAux : AuxState )( z: Z) : State := 
          fun var: Var =>   ( stateAux  ( AuxName var z )). 
                                                                                 
Axiom  getVarValAtState  : Z -> Var -> Num.

(* TODO implemenyta function *)
Inductive isStmtIndMod : stmt_m   -> Z -> Prop :=
 | skipInd : forall z , (isStmtIndMod (SkipInd Invariant_m z) z)
 | affectInd : forall z  var exp  ,     (isStmtIndMod  (AffectInd Invariant_m z var exp) z)
 | seqInd : forall z st1 st2  ,  (isStmtIndMod (SeqInd Invariant_m z st1 st2 ) z)
 | ifInd : forall z cond    st1 st2, (isStmtIndMod ( IfInd Invariant_m  z  cond    st1 st2 ) z)
 | whileInd : forall z inv  cond stmt, 
         (isStmtIndMod (WhileInd Invariant_m  z inv  cond stmt ) z)
.

Reserved Notation "'wpMod' ( a , post )  = pre" (at level 30).
Inductive is_wpMod : stmt_m -> Assertion -> Assertion -> Prop :=
| is_wpMod_caseSkip : forall (post: Assertion) (z : Z) ,  wpMod(( SkipInd  Invariant_m z), post) = post
| is_wpMod_caseAffect :   forall (post: Assertion) (var: Var) ( expr: NumExpr) ( z : Z) , 
  wpMod((AffectInd  Invariant_m z var expr), post)  = (update_assert post var expr ) 
| is_wpMod_caseIf : forall (post : Assertion ) (pre_t : Assertion )(pre_f : Assertion) (st1 st2: stmt_m) (bExp  : NumExpr ) (z : Z) ,
...
| is_wpMod_caseWhile : forall  (post pre_st invariant : Assertion )  (lVar : list Var ) ( bExp : NumExpr ) ( stmt : stmt_m) (z : Z) , 
(* forall  AST, *) (*not sure if a semantic error can be done by commenting it  *) 
(* forall   ST  ,    ( forall v , (   (~ In v lVar)  ->  (* getVarValAtState z v *)  (AuxName v z)   = ( ST v)  ))  -> *)
 ( wpMod(stmt , fun st   => let (s, sAux) := st in
                                                   p_and    (p_forallv (fun v => (p_implies 
                                                                                (p_not (p_in v lVar)) 
                                                                                (p_eq (* getVarValAtState z x *) (sAux (AuxName  v z) )  ( s  v )))))
                                                                      ( invariant st )) = pre_st  ) -> 
       ...
       fun st => 
		...
		(p_and  (  (p_forallv (fun v => (p_implies 
		       ...
		       (p_forallv (fun v => (p_implies 
                                                                                (p_not (p_in v lVar)) 
                                                                                (p_eq (sAux (AuxName  v z)) (* getVarValAtState z x *) ( s v )))))
                           ( p_implies  ( invariant (s , sAux ) ) 
                              ( p_implies                
                                    (p_eq (neval  s sAux   bExp)  0)
                                    	( post ( s , sAux))))))))))
| is_wpMod_caseSeq : forall (post pre_st2 pre_st1 : Assertion ) (st1 st2 : stmt_m ) ( z : Z ),
...
(* 
Axiom semAux : 
forall state aState ( P :  myProp )  lVar  z,
 (isFresh  z P) -> 
...
trivial.
Qed.
(*if the interpretation auxState of the aux variables interpret every aux var (AuxName v z) 
in the same way as (state x) then we can update the values of every prog var in state and 
still the evaluation will be the same    *)
Axiom semAux3 : forall state auxState exp z ,
   ( forall v : Var ,  (auxState (AuxName v z)) = (state v)) ->
	( neval ( FreezeVarAtState state auxState  z ) auxState exp ) = (neval state auxState exp).

Axiom semAux4 : forall state auxState P z ,
   ( forall v : Var ,  (auxState (AuxName v z)) = (state v)) ->
	evalMyProp (P ( ( FreezeVarAtState state auxState  z ) , auxState  ) )  =
	evalMyProp ( P (state, auxState ) ).


Lemma semAux5 : forall state auxState  z ,
   ( forall v : Var ,  (auxState (AuxName v z)) = (state v)) ->
forall v : Var, ( FreezeVarAtState state auxState  z ) v = state v.
Proof.
intros.
...
(* destruct H2. *)
assert (h := IHHexec1 _ _ H5 (H1 (* stateAux *)  s  H0    H n)).
simpl in h.
...
(* wpMod  is monotone *)
Lemma    wpModMon : forall stmtM  Post1 Post2 (stAux : AuxState) Pre1 Pre2 ,
(is_wpMod stmtM Post1 Pre1) ->  
(is_wpMod stmtM Post2 Pre2) -> 
 ( forall (st : State)  , (evalMyProp (Post1 (st, stAux) ) ) -> (evalMyProp (Post2 (st, stAux) )) )   ->  
(forall  (st : State) , ( (evalMyProp (Pre1  (st, stAux)) ) -> (evalMyProp (Pre2 (st, stAux) ))) )   .
...
inversion wpMod1; subst...
inversion wpMod2; subst...
assert (hypIndThen := IHstmtM1 Post1 Post2   stAux pre_t  pre_t0 H8 H10 ).
assert (hypIndElse := IHstmtM2 Post1 Post2  stAux  pre_f  pre_f0 H7 H9 ).
simpl...
simpl in *.
split.
intros.
...
apply ( IHstmtM _ _ stAux _ _ H5 H7 (*H9*) ) .
simpl in *.
trivial.
intros...
...
inversion wpMod1; subst...
inversion wpMod2; subst...
assert ( hypInd2 := IHstmtM2 Post1 Post2 _ pre_st2 pre_st0 H6 H8 H).
assert ( hypInd1 := IHstmtM1 pre_st2 pre_st0 _ Pre1 Pre2 H7 H9 hypInd2).
