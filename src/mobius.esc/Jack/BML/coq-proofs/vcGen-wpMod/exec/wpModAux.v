...
assert (applyIH1 := IHHexec1 _ _  H9  ).
...
assert ( atFinalState := semAux
                               ...
                               (p_and
                                  (p_foralls ( fun s =>  ( p_implies 
                                         ( p_and (invariant0 ( s , sAux) )  
                                         (p_neq (neval s sAux b)  0 ) ) 
                                         ( pre_st  ( s , sAux) )
                                    ) ))
                                     (p_foralls (fun s=>   ( p_implies 
                                         ( p_and (invariant0 (s, sAux))  (p_eq (neval s sAux b)  0 ) ) 
                                         ( post (s, sAux) ))))))
                                    lVar
                                    z
                                  H4  ).
...
assert ( hyp:= semAux1   
                        ...
                        let (s, sAux) := st in
                        p_implies
                        ( p_and (invariant0 (s, sAux) ) (p_neq (neval s sAux b) 0) )
                        ( pre_st (s, sAux)) )
                        lVar
                        z
                        H3
...
simpl in *.
assert( interm :  (forall (stAux : AuxState) (st : State),
       evalMyProp (invariant0 (stateAtLoop st stAux z lVar, stAux)) /\
       neval (stateAtLoop st stAux z lVar) stAux b <> 0 ->
       evalMyProp (pre_st (stateAtLoop st stAux z lVar, stAux))) ).
intros.
destruct H0.
...
assert (hyp3:   forall (aState : AuxState),  ( ( forall (state : State),
       evalMyProp (invariant0 (state, aState)) /\ neval state aState b <> 0 ->
       evalMyProp (pre_st (state, aState)) ))
       /\
            forall (state: State) , (evalMyProp (invariant0 (state, aState)) /\
                 neval state aState b = 0 ->
                 evalMyProp (post  (state , aState )))).
intros.
split.
...
assert (atFinalState1 := atFinalState hyp3).
destruct atFinalState1.
assert ( pre_stIns' := H1 s ).
assert ( pre_stInS :   evalMyProp (pre_st (s, stateAux))). 
apply H1.
split.
assumption.
rewrite <- (progExprEvalProp1 sAux  stateAux s b).
assumption.
assert (applyHI2 := applyIH1 pre_stInS).
(* destruct applyHI2. *)
