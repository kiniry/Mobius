...
Inductive exec_stmt : state -> stmt -> state -> Prop := 
 ...
 | ExecAffect      : forall z s x e, 
     exec_stmt s (Affect z x e) (update s x (eval_expr s e))

 | ExecIf_true     : forall z s1 s2 e stmtT stmtF,
     eval_expr s1 e <> 0 -> 
     exec_stmt s1 stmtT s2 ->
     exec_stmt s1 (If  z e stmtT stmtF) s2

 | ExecIf_false    : forall z s1 s2 e stmtT stmtF,
     eval_expr s1 e = 0 -> 
     exec_stmt s1 stmtF s2 ->
     exec_stmt s1 (If z e stmtT stmtF) s2

 | ExecWhile_true  : forall z s1 s2 s3 e inv stmt,
     eval_expr s1 e <> 0 ->
     exec_stmt s1 stmt s2 ->
     exec_stmt s2 (While z e inv stmt) s3 ->
     exec_stmt s1 (While z e inv stmt) s3

 | ExecWhile_false  : forall z s1 e inv stmt,
     eval_expr s1 e = 0 ->
     exec_stmt s1 (While z e inv stmt) s1
 
 | ExecSnil : forall z s, exec_stmt s ( Snil z ) s

 | ExecSseq   : forall z  s1 s2 s3 i stmt,
      exec_stmt s1 i s2 -> 
      exec_stmt s2 stmt s3 -> 
      exec_stmt s1 (Sseq z i stmt) s3.
