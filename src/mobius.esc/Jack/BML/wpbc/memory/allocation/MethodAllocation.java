...
import bytecode.objectmanipulation.BCInvoke;
...
public class MethodAllocation {
	...
	public static int getMethodAllocates(BCMethod method)
		...
		/*if (method.isChecked()) {
			Util
					.dump(method + " allocations known  "
							+ method.getAllocations());
			if (method.toString().equals("java.lang.String.<init>  ([CII)")) {
				Util.dump("NB : OURRR  method " + method + " allocates  "
						+ method.getAllocations());
			}
		...
		//		//debug
		//		if ( method.toString().equals("java.lang.String.<init>([CII)V")){
		//			Util.dump( "NB : method " + method + " allocates " +
		// method.getAllocations() );
		//		}
	...
	private static int getMethodAllocated(BCInstructionAlloc bcalloc,
		...
		if (instr instanceof BCInvoke) {
			bcalloc.setChecked();
			BCMethod m = ((BCInvoke) instr).getInvokedMethod();
			Util.dump(method + " invokes " + m.toString() + " that allocates ");
			m.initMethod();
			//if (
			// method.toString().equals("java.lang.Integer.toUnsignedString(II)Ljava/lang/String;")
			// ) {
			//	}
