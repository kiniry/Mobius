...
public class TriaryForm extends Formula {
	...
	/**
	 * Constructs a question formula form a loaded jpo file.
	...
	public boolean is(Formula f) {
		return f instanceof TriaryForm
			&& getNodeType() == ((TriaryForm) f).getNodeType()
			&& f1.is(((TriaryForm) f).f1)
			&& left.is(((TriaryForm) f).left)
			&& right.is(((TriaryForm) f).right);
	}
