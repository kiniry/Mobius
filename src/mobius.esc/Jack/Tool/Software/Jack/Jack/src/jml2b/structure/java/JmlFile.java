...
public class JmlFile implements Serializable, IJmlFile {
	...
	public int getNbPoProved(String prover) {
		...
		Enumeration e = classes.elements();
		while (e.hasMoreElements()) {
			res += ((Class) e.nextElement()).nbPoProved(prover);
		}
		return res;
	}
	...
	public int getNbPoProved() {
		...
		Enumeration e = classes.elements();
		while (e.hasMoreElements()) {
			res += ((Class) e.nextElement()).nbPoProved();
		}
		return res;
	}
	...
	public void mergeWith(jpov.structure.JmlFile jf) {
		Enumeration e = classes.elements();
		while (e.hasMoreElements()) {
			((Class) e.nextElement()).mergeWith( jf.getClasses());
		}
	}
...
//	}
	...
	public String emit(IJml2bConfiguration config) {
		String s = "package " + filePackage.getFullyQualifiedName() + ";\n\n";

		Enumeration e = importedPackages.elements();
		while (e.hasMoreElements()) {
			Package element = (Package) e.nextElement();
			s += "import " + element.getFullyQualifiedName() + ".*;\n";
		}
		...
		while (i.hasNext()) {
			...
			s += "import "
				+ ((Package) importedClasses.get(element))
		...
		e = classes.elements();
		while (e.hasMoreElements()) {
			Class element = (Class) e.nextElement();
	...
	public void purgeObvious() {
		Enumeration e = classes.elements();
		while (e.hasMoreElements()) {
			((Class) e.nextElement()).purgeObvious();
		}
	}
	...
	public void save(IJml2bConfiguration config, JpoOutputStream s) throws IOException {
		s.writeInt(JPO_FILE_FORMAT_VERSION);
		s.writeUTF(getFileNameAbsolutePath());
		s.writeInt(getNbPo());
		s.writeInt(getNbPoProved());
		s.writeInt(Languages.getNbLanguages());
		Enumeration e = Languages.getLanguagesNames();
		while (e.hasMoreElements()) {
			String element = (String) e.nextElement();
			s.writeUTF(element);
			s.writeBoolean(config.isFileGenerated(element));
			if (Languages.getProverStatusClass(element) == null)
				s.writeInt(0);
			else
				s.writeInt(getNbPoProved(element));
		}
		s.writeInt(getDepends().size());
		e = getDepends().elements();
		while (e.hasMoreElements()) {
			String element = (String) e.nextElement();
			s.writeUTF(element);
		}
