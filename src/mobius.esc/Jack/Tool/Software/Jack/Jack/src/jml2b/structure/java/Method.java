...
//		}
	...
	public  int nbPoChecked() {
		return lemmas.nbPoChecked() + wellDefinednessLemmas.nbPoChecked();

	}

	public int nbPo() {
		return lemmas.nbPo() + wellDefinednessLemmas.nbPo();
	}

	void purgeObvious() {
		lemmas.proveObvious(true, true);
		wellDefinednessLemmas.proveObvious(true, true);
	}

	public int nbPoProved(String prover) {
		return lemmas.nbPoProved(prover)
			+ wellDefinednessLemmas.nbPoProved(prover);
	}

	public int nbPoProved() {
		//		if (lemmas == null)
		//			return 0;
		return lemmas.nbPoProved() + wellDefinednessLemmas.nbPoProved();
	}

	void mergeWith(jpov.structure.Method jf) {
		lemmas.mergeWith(jf.getLemmas());
		jf.freeLemmas();
		if (wellDefinednessLemmas != null)
			wellDefinednessLemmas.mergeWith(jf.getWellDefinednessLemmas());
	}

	void mergeWith(jpov.structure.Method[] jf) {
		for (int i = 0; i < jf.length; i++) {
			jpov.structure.Method m = jf[i];
	...
	public String getInfo() {
		String res = getFullyQualifiedName();
		res += signature.toJavaWithParameterName();
		if (requires != null)
			res += "\nrequires " + requires.toJava(9) + ";";
		if (specCases.size() != 0) {
			res += "\n{|";
			Enumeration e = specCases.elements();
			boolean first = true;
	...
	public String emitSpecCase(IJml2bConfiguration config) {
		String s = "    /*@\n";
		if (requires != null)
	...
	String emit(IJml2bConfiguration config) {
		String s = emitSpecCase(config) + "\n";
		s += getModifiers().emit();
		s += (returnType != null ? returnType.toJava() : "") + " " + name + "(";
		s += signature.emit();
		s += ")";
		if (hasNoCode())
			return s + ";\n";
	...
	private static final int ALREADY_NOT_ANNOTATED = 2;
	...
	public Collection annotate(IJml2bConfiguration config)
		throws Jml2bException, PogException {
		Vector fileUpdates = new Vector();
		if (isAnnotated != NOT_ANNOTATED) {
			if (getFirstLine() != 0 && isAnnotated == ALREADY_ANNOTATED)
				fileUpdates.add(
					new ReplaceFileUpdate(
						getJmlFile().getFileName().getFile(),
						emitSpecCase(config),
						getFirstLine() - 1));
			return fileUpdates;
		}
		if (body != null) {
			ModifiableSet modifies = new ModifiableSet();
			body.getPrecondition(
				config,
				modifies,
				annotatedPrecondition,
				annotatedPostcondition);
			annotatedModifies = modifies.getSet();
			Expression req = null;
			if (annotatedPrecondition.size() != 0) {
				Enumeration e = annotatedPrecondition.elements();
				while (e.hasMoreElements()) {
					Expression element = (Expression) e.nextElement();
					if (req == null)
						req = element;
					else
						req = new BinaryExp(LOGICAL_OP, req, "&&", element);
				}
			} else
				req = null;
			Expression ens = null;
			if (annotatedPostcondition.size() != 0) {
				Enumeration e = annotatedPostcondition.elements();
				while (e.hasMoreElements()) {
					Expression element = (Expression) e.nextElement();
					if (ens == null)
						ens = element;
					else
						ens = new BinaryExp(LOGICAL_OP, ens, "&&", element);
				}
			} else
				ens = null;
			...
			if (annotatedModifies.size() == 0)
				mod = null;
			else {
				mod = new ModifiesList(annotatedModifies.iterator());
			}
			isAnnotated = ALREADY_NOT_ANNOTATED;
			if (addAnnotation(config, req, ens, mod))
				if (getFirstLine() != 0) {
					fileUpdates.add(
						new ReplaceFileUpdate(
							getJmlFile().getFileName().getFile(),
	...
	public void save(IJml2bConfiguration config, JpoOutputStream s, JmlFile jf) throws IOException {
		...
		s.writeUTF(getPmiName() == null ? "" : getPmiName());
		s.writeInt(getFirstLine());
		s.writeInt(nbPo());
		s.writeInt(nbPoProved());
		s.writeInt(nbPoChecked());
		getLemmas().save(config, s, jf);
		getWellDefinednessLemmas().save(config, s, jf);
	}
