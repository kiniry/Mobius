...
public class TraceUtils {
	...
	public static Vector initLoopInstructions(Path path, BCInstruction instr,
			Vector backEdges, BCInstruction[] instrs, Vector loopStarts)
			throws IllegalLoopException {
...
//				BCInstruction prevStart = seqInstr.getPrev();
				...
				BCLoopStart loopStart = wrapLoopStart(seqInstr, instr
						.getPosition(), backEdges, loopStarts);
				...
				initLoopInstructions(path, srt, backEdges, instrs, loopStarts);
				return backEdges;
			}
			initLoopInstructions(path, seqInstr, backEdges, instrs, loopStarts);
			initLoopInstructions(path, srt, backEdges, instrs, loopStarts);
			return backEdges;
...
//				BCInstruction nextLast = branchInstr.getNext();
				...
				// seq branch
				seqInstr.setPrev(null);
				initLoopInstructions(path, branchTargetInstr, backEdges, instrs, loopStarts);
				BCLoopStart loopStart = wrapLoopStart(seqInstr, instr
						.getPosition(), backEdges, loopStarts);
				if (nextStart != null) {
					nextStart.setPrev(loopStart);
			...
			initLoopInstructions(path, target, backEdges, instrs, loopStarts);
			return backEdges;
			...
			if ((next != null) && (path.contains(next))) {
				...
				BCLoopStart loopStart = wrapLoopStart(next,
						instr.getPosition(), backEdges , loopStarts);
				if (nextStart != null) {
					nextStart.setPrev(loopStart);
				}
				Util.update(instrs, loopStart);
			...
			initLoopInstructions(path, next, backEdges, instrs, loopStarts);
			return backEdges;
		}
		//continue recursively
	}
	...
	private static BCLoopStart wrapLoopStart(BCInstruction startLoop,
			int loopEndPosition, Vector backedges, Vector loopStarts) {
	...
	/**
	 * @param entryBlock
	 * @param instructions@param loopStarts
	 * @throws IllegalLoopException
	 */
	public static void initLoopInstructions(BCInstruction entryPoint,
			BCInstruction[] instructions, Vector loopStarts) throws IllegalLoopException {
		loopNumber = 0;
		initLoopInstructions(new Path(), entryPoint, new Vector(), instructions, loopStarts);
	}
