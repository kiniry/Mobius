...
public class BCIFLT extends BCConditionalBranch {
	...
	public Formula wpBranch(
		...
		(Formula)_normal_Postcondition.substitute(
				...
				Expression.getCOUNTER_MINUS_1());
		wp =
			Formula.getFormula(stackTop_less_0, less_branch, Connector.IMPLIES);
...
//		 in case of executing next instruction S(t) >= 0
		...
		Integer key = vcs.addHyp(getPosition(), stackTop_not_less_0);
	...
	public VCGPath wpBranch(IJml2bConfiguration config, VCGPath vcs, ExsuresTable _exc) {
		...
		vcs.substitute(
				Expression.COUNTER,
				Expression.getCOUNTER_MINUS_1());
		Integer key = vcs.addHyp(getPosition(), stackTop_less_0);
		vcs.addHypsToVCs(key);
		return vcs;
	}
