...
import bytecode_wp.constants.BCCONSTANT_LITERAL;
 ...
 * the runtime constant pool of the current class. The runtime constant pool
 * entry at index either must be a runtime constant of type int or float, or
 * must be a symbolic reference to a string literal. If the runtime constant
 * pool entry is a runtime constant of type int or float, the numeric value of
 * that runtime constant is pushed onto the operad stack as an int or float,
 * respectively.
 * 
 * Otherwise, the runtime constant pool entry must be a reference to an instance
 * of class String representing a string literal. A reference to that instance,
 * value, is pushed onto the operand stack.
 * 
 * 
 * Notes
 * 
 * The ldc instruction can only be used to push a value of type float taken from
 * the float value set because a constant of type float in the constant pool
 * must be taken from the float value set. psi^n[ t <-- t+1] [S(t+1) <--
 * cp_at(index)]
 */
...
public class BCLDC extends BCInstruction implements BCCPInstruction {
	...
	public Formula wp(IJml2bConfiguration config,
		...
		wp = (Formula) wp
				.substitute(top_stack_plus_1,
						((BCCONSTANT_LITERAL) getConstantPool().getConstant(
								getIndex())).getLiteral());
		return wp;
	}
	...
	public VCGPath wp(IJml2bConfiguration config, VCGPath vcs, ExsuresTable _exc) {
		...
		Stack top_stack_plus_1 = new Stack(Expression.getCOUNTER_PLUS_1());
		if (getConstantPool().getConstant(getIndex()) instanceof BCCONSTANT_String) {
			vcs.substitute(top_stack_plus_1,
					((BCCONSTANT_String) getConstantPool().getConstant(
							getIndex())).getLiteral());
		}
		if (getConstantPool().getConstant(getIndex()) instanceof BCCONSTANT_Integer) {
			vcs.substitute(top_stack_plus_1,
					((BCCONSTANT_Integer) getConstantPool().getConstant(
							getIndex())).getLiteral());
		}
		return vcs;
	}
