...
public class BCPUTFIELD extends BCFieldOrMethodInstruction {
	...
	public VCGPath wp(IJml2bConfiguration config, VCGPath vcs, ExsuresTable _exc) {
		...
		Integer key1 = vcs.addHyp(getPosition(), objNotNull);
		...
		VCGPath excPost =
			getWpForException(config,getExceptionsThrown()[0]);

		excPost.setInstrIndex( getPosition());
		//S(t -1) == null ==> psi^e("Ljava/lang/NullPointerException;")
		
		Integer key2 = excPost.addHyp(getPosition(), objNull);
		excPost.addHypsToVCs(key2); 
		vcs.merge(excPost);
		return vcs;
	}
