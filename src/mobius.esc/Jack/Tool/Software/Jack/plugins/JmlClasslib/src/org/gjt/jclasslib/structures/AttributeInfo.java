...
import org.gjt.jclasslib.structures.iflowattributes.ExceptionSafeAttribute;

/**
    Base class for all attribute structures in the <tt>attribute</tt> package.

    @author <a href="mailto:jclasslib@ej-technologies.com">Ingo Kegel</a>
    @version $Revision: 1.3 $ $Date: 2005/03/24 10:29:57 $
*/
	...
	*/
	public static AttributeInfo createOrSkip(DataInput in, ClassFile classFile)
		...
		if (Boolean.getBoolean(SYSTEM_PROPERTY_SKIP_ATTRIBUTES)) {
			...
			ConstantUtf8Info cpInfoName =
				classFile.getConstantPoolUtf8Entry(attributeNameIndex);
			...
			if (ConstantValueAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				...
				ExceptionsAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new ExceptionsAttribute();

			} else if (
				InnerClassesAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new InnerClassesAttribute();

			} else if (
				SyntheticAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new SyntheticAttribute();

			} else if (
				SourceFileAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new SourceFileAttribute();

			} else if (
				LineNumberTableAttribute.ATTRIBUTE_NAME.equals(
					attributeName)) {
				attributeInfo = new LineNumberTableAttribute();

			} else if (
				LocalVariableTableAttribute.ATTRIBUTE_NAME.equals(
					attributeName)) {
				attributeInfo = new LocalVariableTableAttribute();

			} else if (
				DeprecatedAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new DeprecatedAttribute();
			} else if (
				MethodSpecificationAttribute.ATTRIBUTE_NAME.equals(
					attributeName)) {
				attributeInfo =
					new MethodSpecificationAttribute(attributeLength);
			} else if (AssertAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new AssertAttribute(attributeLength);
			} else if (
				LoopSpecificationAttribute.ATTRIBUTE_NAME.equals(
					attributeName)) {
				attributeInfo = new LoopSpecificationAttribute(attributeLength);
			} else if (
				BlockSpecificationAttribute.ATTRIBUTE_NAME.equals(
					attributeName)) {
				attributeInfo =
					new BlockSpecificationAttribute(attributeLength);
			} else if (
				ModelFieldAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
			    attributeInfo = new ModelFieldAttribute(attributeLength);
			} else if (
				ClassInvariantAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new ClassInvariantAttribute(attributeLength);
			} else if (
				ConstraintAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
				attributeInfo = new ConstraintAttribute(attributeLength);
			} else if (SecLevelFieldAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
			    attributeInfo = new SecLevelFieldAttribute(attributeLength);
			} else if (SecLevelMethodAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
			    attributeInfo = new SecLevelMethodAttribute(attributeLength);
			} else if (ExceptionSafeAttribute.ATTRIBUTE_NAME.equals(attributeName)) {
			    attributeInfo = new ExceptionSafeAttribute(attributeLength);
			} else {
	...
	protected AttributeInfo(int attributeLength) {
