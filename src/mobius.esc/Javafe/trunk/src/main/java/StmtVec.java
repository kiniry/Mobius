 ...
 ** $Id: StmtVec.j 1928 2006-05-19 12:43:10Z chalin $
 **
 ** DO NOT EDIT!!<p>
 **/
 ...
 *
 * Stmt should be globally replaced with the simple name of the
 *		element type<p>
 ...
 *
 * The resulting type will have name StmtVec.<p>
 * 
 ...
 *
 *
 * By Default, StmtVec's may not contain nulls.  If the ability to
...
public class  StmtVec {
    ...
    private /*@ spec_public non_null*/ Stmt[] elements;
    //@ invariant (\forall int i; (0<=i && i<count) ==> elements[i]!=null);
    //@ invariant \typeof(elements) == \type(Stmt[]);
	...
	this.elements = new Stmt[(cnt == 0 ? 2 : cnt)];
	//@ set elements.owner = this;
	...
	return new StmtVec(count);
    }

    //@ requires vec!=null;
    //@ requires vec.elementType <: \type(Stmt);
    //@ requires !vec.containsNull;
    //@ ensures \result!=null;
    //@ ensures \fresh(\result);
    public static StmtVec make(Vector vec) {
	int sz = vec.size();
	StmtVec result = new StmtVec(sz);
	result.count = sz;
	...
	return new StmtVec(els);
    }

    //
    //@ requires s.vectorCount>1;
    //@ requires s.elementType <: \type(Stmt);
    //@ ensures \result!=null;
    ...
    public static StmtVec popFromStackVector(/*@non_null*/ StackVector s) {
	// Creates a new StmtVec from top stuff in StackVector
	int sz = s.size();
	StmtVec r = new StmtVec(sz);
  ...
  public final StmtVec copy() {
    StmtVec result = new StmtVec(count);
    result.count = count;
  ...
  public final void addElement(Stmt x) {
    if( count == elements.length ) {
      Stmt[] newElements = new Stmt[ 2*(elements.length==0 ?
					      2 : elements.length) ];
