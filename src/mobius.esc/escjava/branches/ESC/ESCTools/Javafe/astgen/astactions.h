...
/**
   ...
   spec.  Before the first ClassDeclaration there must be a line with
   the following lexical structure:
...
/* The following procedures must implement state transitions as indicated
   above.  They may have other side effects, such as outputing text.
   The implementors may infer appropriate pre-conditions from the
   state transition diagram above, for example, "abstract" will not
   be called unless the current state is "ABOVECLASS".
...
/* With two exceptions, every piece of text in the input file (except
   ...
   echo routines.  This includes text that's also passed to state
   transition procedures such as "classname" and
   "supername"; in these cases, astecho is called first, then the
   state transition routine.  One exception to the rule that every
   piece of text is sent to an echo routine is the line containing the
   "//# EndHeader" section, which triggers the call to "endheader".
   The other exception is the '}' character that ends a (top-level)
   class declaration; this triggers the call to "endclass", to which
   the closing "}" plus any characters matching "{white-space}\n{0,1}"
   is passed.
