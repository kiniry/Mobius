...
# fwdwizard pragma (if any) at location 'line', 'col' for each input line:
#   source-filename line col
# A "fwdwizard pragma" has the form:
#   /*@(fwdwizard:...) ... */
...
# of the second ellipsis.  The fwdwizard pragma is changed to
#   /* REMOVED @(fwdwizard:...) ... BECAUSE warning */
...
while (<STDIN>) {
    ...
    # print STDOUT "input $file  $line $col \n";
    ...
    # print STDOUT  "LEFT <$strLeft>\n";
    ($strLeft =~ /(^.*\/\*)(\#\(rccwizard:[^\)]*\) $)/) || next;
    $newLeft = $1 . " REMOVED " . $2;

    # print STDOUT  "right $strRight\n";
    ($strRight =~ /(.*?)(\*\/.*$)/) ||
