 ...
 */
public abstract class AESC extends AEscjavaAction {
  ...
  protected boolean doit(final IAdaptable o) throws Exception {
    ...
    if (o instanceof ICompilationUnit) {
      ...
      final IResource resource = p.getCorrespondingResource();
      if (resource == null) {
        return false;
      }
      action(resource);
   ...
   */
  protected abstract void action(IResource r);
  
  @Override
  protected boolean end(final IJavaProject jp, final Collection<IAdaptable> elements) {
    final Collection<IResource> touchList = touch;
    // FIXME - is this the right thread to use
    SafeRunner.run(new SafeRunnable() {
      public void run() throws Exception {
        for (IResource r: touchList) {
          r.touch(null);
        }
      }
    });
