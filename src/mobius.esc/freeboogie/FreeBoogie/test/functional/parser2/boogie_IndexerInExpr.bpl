...
procedure Test.Foo$System.Int32.array$notnull(stuff$in : ref where $IsNotNull(stuff$in, ValueArray(System.Int32, 1)));
  free requires ($Heap[stuff$in, $allocated] == true);
  requires ((0 < $Length(stuff$in)) && (0 <= ValueArrayGet($Heap[stuff$in, $elements], 0)));
  requires (((($Heap[stuff$in, $ownerFrame] == $PeerGroupPlaceholder) || !($Heap[$Heap[stuff$in, $ownerRef], $inv] <: $Heap[stuff$in, $ownerFrame])) || ($Heap[$Heap[stuff$in, $ownerRef], $localinv] == $BaseClass($Heap[stuff$in, $ownerFrame]))) && (forall $pc : ref :: ((((($pc != null) && ($Heap[$pc, $allocated] == true)) && ($Heap[$pc, $ownerRef] == $Heap[stuff$in, $ownerRef])) && ($Heap[$pc, $ownerFrame] == $Heap[stuff$in, $ownerFrame])) ==> (($Heap[$pc, $inv] == $typeof($pc)) && ($Heap[$pc, $localinv] == $typeof($pc))))));
...
implementation Test.Foo$System.Int32.array$notnull(stuff$in : ref) {
  var stuff : ref where $IsNotNull(stuff, ValueArray(System.Int32, 1));
  var stack0o : ref;
  entry: stuff := stuff$in; goto block3995;
  block3995: goto block4216;
  block4216: goto block4233;
  block4233: assert (ValueArrayGet($Heap[stuff, $elements], 0) != -1); goto block4386;
